      ...
      ** this (that the method has already been called). */
      ...
      if( eType==SQLITE_INTEGER || eType==SQLITE_FLOAT ){
        ...
        if( eType==SQLITE_TEXT ){
          ...
          z = static_cast<const u8 *>(sqlite3_value_blob(pVal));
        }
...
static int sessionValueSetStr(
  ...
  ** error. Hence the code to create the copy externally. */
  u8 *aCopy = static_cast<u8 *>(sqlite3_malloc64((sqlite3_int64)nData+1));
...
static int sessionUpdateFind(
  ...
  if( p->aUpdateMask==0 ){
    p->aUpdateMask = static_cast<u32 *>(sqlite3_malloc(nU32*sizeof(u32)));
    if( p->aUpdateMask==0 ){
      rc = SQLITE_NOMEM;
    }
  }
...
static int sessionChangesetFindTable(
  ...
  *ppTab = 0;
  ...
  if( !pTab ){
    ...
    pTab = static_cast<SessionTable *>(sqlite3_malloc64(sizeof(SessionTable) + nCol + nTab+1));
    if( !pTab ){
      return SQLITE_NOMEM;
    }
    memset(pTab, 0, sizeof(SessionTable));
...
int sqlite3rebaser_create(sqlite3_rebaser **ppNew){
  ...
  pNew = static_cast<sqlite3_rebaser *>(sqlite3_malloc(sizeof(sqlite3_rebaser)));
  if( pNew==0 ){
    rc = SQLITE_NOMEM;
  }else{
