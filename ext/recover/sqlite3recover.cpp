...
/*
** Declaration for public API function in file dbdata.cpp. This may be called
  ...
  if( pgno==0 ){
    ...
    if( pStmt ){
      ...
      if( SQLITE_ROW==sqlite3_step(pStmt) ){
        ...
        aPg = (const u8*)sqlite3_column_blob(pStmt, 0);
...
static void recoverAddTable(
  ...
  if( pStmt ){
    ...
    pNew = (RecoverTable*)recoverMalloc(p, nByte);
    if( pNew ){
      int i = 0;
      int iField = 0;
      char *csr = 0;
