...
** Implementation note:
**
** Much of the tokenizer logic is copied out of the tokenize.cpp source file
...
char *sqlite3_normalize(const char *zSql){
  ...
  ** SELECT statement into "in(?,?,?)" */
  for(i=0; i<j; i=n){
    ...
    if( k<5 ){
      z = (char*)sqlite3_realloc64(z, j+(5-k)+1);
      if( z==0 ) return 0;
...
int main(int argc, char **argv){
  ...
  for(i=1; i<argc; i++){
    ...
    sz = ftell(in);
    rewind(in);
    zBuf = (char*)sqlite3_realloc64(zBuf, sz+1);
    if( zBuf==0 ){
