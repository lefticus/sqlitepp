...
static int zipfileOpen(sqlite3_vtab *p, sqlite3_vtab_cursor **ppCsr){
  ...
  ZipfileCsr *pCsr;
  pCsr = (ZipfileCsr*)sqlite3_malloc(sizeof(*pCsr));
...
static void zipfileInflate(
  ...
  u8 *aRes = (u8*)sqlite3_malloc(nOut);
  if( aRes==0 ){
      ...
      if( i==4 || pCDS->iCompression==0 || pCDS->iCompression==8 ){
        ...
        if( szFinal>0 ){
          ...
          if( pCsr->pCurrent->aData ){
            ...
            aBuf = aFree = (u8*)sqlite3_malloc64(sz);
            if( aBuf==0 ){
              rc = SQLITE_NOMEM;
            }else{
...
static ZipfileEntry *zipfileNewEntry(const char *zPath){
  ZipfileEntry *pNew;
  pNew = (ZipfileEntry*)sqlite3_malloc(sizeof(ZipfileEntry));
  if( pNew ){
    memset(pNew, 0, sizeof(ZipfileEntry));
    ...
    if( rc==SQLITE_OK ){
      if( sqlite3_value_type(apVal[7])==SQLITE_NULL ){
        ...
        ** a regular file or a symlink. */
        const u8 *aIn = (const u8*)sqlite3_value_blob(apVal[7]);
...
static int zipfileBufferGrow(ZipfileBuffer *pBuf, i64 nByte){
  if( pBuf->n+nByte>pBuf->nAlloc ){
    ...
    aNew = (u8*)sqlite3_realloc64(pBuf->a, nNew);
...
static void zipfileStep(sqlite3_context *pCtx, int nVal, sqlite3_value **apVal){
  ...
  ** be deflated or simply stored in the zip archive. */
  if( sqlite3_value_type(pData)==SQLITE_NULL ){
    ...
    aData = (const u8*)sqlite3_value_blob(pData);
