...
**
**   (linux)    gcc -fPIC -shared tmstmpvfs.cpp -o tmstmpvfs.so
**   (mac)      clang -fPIC -dynamiclib tmstmpvfs.cpp -o tmstmpvfs.dylib
**   (windows)  cl tmstmpvfs.cpp -link -dll -out:tmstmpvfs.dll
**
...
** the content of one or more tmstmpvfs.cpp log files.  If all of the
...
static int tmstmpOpen(
  ...
  if( pDb!=0 ){
    ...
    p->isDb = 1;
    r1 = 0;
    pLog = (TmstmpLog*)sqlite3_malloc64( sizeof(TmstmpLog) );
    if( pLog==0 ){
