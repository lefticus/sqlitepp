...
FILE *sqlite3_fopen(const char *zFilename, const char *zMode){
  ...
  b1 = (wchar_t*)sqlite3_malloc( (sz1+1)*sizeof(b1[0]) );
  b2 = (wchar_t*)sqlite3_malloc( (sz2+1)*sizeof(b1[0]) );
  if( b1 && b2 ){
...
char *sqlite3_fgets(char *buf, int sz, FILE *in){
  if( UseWtextForInput(in) ){
    ...
    ** into '?'.
    */
    wchar_t *b1 = (wchar_t*)sqlite3_malloc( sz*sizeof(wchar_t) );
    if( b1==0 ) return 0;
...
int sqlite3_fputs(const char *z, FILE *out){
  if( !UseWtextForOutput(out) ){
    ...
    */
    int sz = (int)strlen(z);
    wchar_t *b1 = (wchar_t*)sqlite3_malloc( (sz+1)*sizeof(wchar_t) );
    if( b1==0 ) return 0;
