...
static int delta_create(
  ...
  */
  nHash = lenSrc/NHASH;
  collide = (int*)sqlite3_malloc64( (sqlite3_int64)nHash*2*sizeof(int) );
...
static void deltaCreateFunc(
  ...
  aOut = (char*)sqlite3_malloc64(nNew+70);
  if( aOut==0 ){
...
static void deltaApplyFunc(
  ...
  aOut = (char*)sqlite3_malloc64((sqlite3_int64)nOut+1);
  if( aOut==0 ){
    sqlite3_result_error_nomem(context);
  }else{
...
#define DELTAPARSEVTAB_DELTA  3
  if( rc==SQLITE_OK ){
    pNew = (deltaparsevtab_vtab*)sqlite3_malloc64( sizeof(*pNew) );
...
static int deltaparsevtabOpen(sqlite3_vtab *p, sqlite3_vtab_cursor **ppCursor){
  ...
  pCur = (deltaparsevtab_cursor*)sqlite3_malloc( sizeof(*pCur) );
...
static int deltaparsevtabFilter(
  ...
  pCur->aDelta = (char*)sqlite3_malloc64( pCur->nDelta+1 );
  if( pCur->aDelta==0 ){
    pCur->nDelta = 0;
    return SQLITE_NOMEM;
  }
