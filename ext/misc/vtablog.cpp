...
**
**   (linux)    gcc -fPIC -shared vtablog.cpp -o vtablog.so
**   (mac)      clang -fPIC -dynamiclib vtablog.cpp -o vtablog.dylib
**   (windows)  cl vtablog.cpp -link -dll -out:vtablog.dll
**
** USAGE EXAMPLE:
**
**     .load ./vtablog
...
static int vtablogConnectCreate(
  ...
  if( rc==SQLITE_OK ){
    pNew = (vtablog_vtab*)sqlite3_malloc( sizeof(*pNew) );
...
static int vtablogOpen(sqlite3_vtab *p, sqlite3_vtab_cursor **ppCursor){
  ...
  printf("%s.%s.xOpen(cursor=%d)\n", pTab->zDb, pTab->zName,
         ++pTab->nCursor);
  pCur = (vtablog_cursor*)sqlite3_malloc( sizeof(*pCur) );
