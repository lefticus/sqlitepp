...
static Decimal *decimalNewFromText(const char *zIn, int n){
  ...
  if( iExp>0 ){
    ...
    if( iExp>0 ){   
      signed char *a = (signed char*)sqlite3_realloc64(p->a,
                                     (sqlite3_int64)p->nDigit
...
static Decimal *decimal_new(
  ...
  switch( eType ){
    ...
    case SQLITE_BLOB: {
      ...
      x = (const unsigned char*)sqlite3_value_blob(pIn);
...
static void decimal_result(sqlite3_context *pCtx, Decimal *p){
  ...
  z = (char*)sqlite3_malloc64( (sqlite3_int64)p->nDigit+4 );
  if( z==0 ){
    sqlite3_result_error_nomem(pCtx);
    return;
  }
...
static void decimal_result_sci(sqlite3_context *pCtx, Decimal *p, int N){
  ...
  nDigit -= nZero;
  z = (char*)sqlite3_malloc64( (sqlite3_int64)nDigit+20 );
  if( z==0 ){
    sqlite3_result_error_nomem(pCtx);
    return;
  }
...
static void decimal_expand(Decimal *p, int nDigit, int nFrac){
  ...
  a = (signed char*)sqlite3_realloc64(p->a, nDigit+1);
  if( a==0 ){
    p->oom = 1;
    return;
  }
...
static void decimalMul(Decimal *pA, Decimal *pB){
  ...
  acc = (signed char*)sqlite3_malloc64( sumDigit );
  if( acc==0 ){
    pA->oom = 1;
    goto mul_end;
  }
