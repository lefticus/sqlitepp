...
** COMPILE:
**
**   gcc -Wall -g -shared -fPIC -I. qpvtab.cpp -o qqvtab.so
**
** EXAMPLE USAGE:
**
**   .load ./qpvtab
...
#define QPVTAB_NONE   12
  if( rc==SQLITE_OK ){
    pNew = (qpvtab_vtab*)sqlite3_malloc( sizeof(*pNew) );
...
static int qpvtabOpen(sqlite3_vtab *p, sqlite3_vtab_cursor **ppCursor){
  qpvtab_cursor *pCur;
  pCur = (qpvtab_cursor*)sqlite3_malloc( sizeof(*pCur) );
...
static void qpvtabStrAppendValue(
  ...
  switch( sqlite3_value_type(pVal) ){
    ...
    case SQLITE_BLOB: {
      int i;
      const unsigned char *a = (const unsigned char*)sqlite3_value_blob(pVal);
      int n = sqlite3_value_bytes(pVal);
