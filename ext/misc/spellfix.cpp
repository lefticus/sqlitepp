...
static unsigned char *phoneticHash(const unsigned char *zIn, int nIn){
  unsigned char *zOut = (unsigned char*)sqlite3_malloc64( nIn + 1 );
  int i;
  int nOut = 0;
  char cPrev = 0x77;
  char cPrevX = 0x77;
...
static int editdist1(const char *zA, const char *zB, int *pnMatch){
  ...
  if( nB<(sizeof(mStack)*4)/(sizeof(mStack[0])*5) ){
    ...
    m = toFree = (int*)sqlite3_malloc64( (nB+1)*5*sizeof(m[0])/4 );
    if( m==0 ) return -3;
  }
  cx = (char*)&m[nB+1];
...
static int editDist3ConfigLoad(
  ...
  while( sqlite3_step(pStmt)==SQLITE_ROW ){
    ...
    if( pLang==0 || iLang!=iLangPrev ){
      EditDist3Lang *pNew;
      pNew = (EditDist3Lang*)sqlite3_realloc64(p->a, (p->nLang+1)*sizeof(p->a[0]));
    ...
    if( nFrom==1 && zFrom[0]=='?' && nTo==0 ){
      ...
      pCost = (EditDist3Cost*)sqlite3_malloc64( sizeof(*pCost) + nExtra );
...
static EditDist3FromString *editDist3FromStringNew(
  ...
  pStr = (EditDist3FromString*)sqlite3_malloc64( sizeof(*pStr) + sizeof(pStr->a[0])*n + n + 1 );
  if( pStr==0 ) return 0;
  ...
  for(i=0; i<n; i++){
    ...
    for(p=pLang->pCost; p; p=p->pNext){
      ...
      if( p->nTo==0 ){
        apNew = (EditDist3Cost**)sqlite3_realloc64(pFrom->apDel,
                                sizeof(*apNew)*(pFrom->nDel+1));
        if( apNew==0 ) break;
        pFrom->apDel = apNew;
        apNew[pFrom->nDel++] = p;
      }else{
        apNew = (EditDist3Cost**)sqlite3_realloc64(pFrom->apSubst,
...
static int editDist3Core(
  ...
  if( nByte<=sizeof(stackSpace) ){
    ...
    m = pToFree = (unsigned int*)sqlite3_malloc64( nByte );
  ...
  for(i2=0; i2<n2; i2++){
    ...
    for(p=pLang->pCost; p; p=p->pNext){
      ...
      a2[i2].nIns++;
      apNew = (EditDist3Cost**)sqlite3_realloc64(a2[i2].apIns, sizeof(*apNew)*a2[i2].nIns);
      if( apNew==0 ){
        res = -1;  /* Out of memory */
        goto editDist3Abort;
      }
...
static int editDist3Install(sqlite3 *db){
  int rc;
  EditDist3Config *pConfig = (EditDist3Config*)sqlite3_malloc64( sizeof(*pConfig) );
...
#ifdef SQLITE_SPELLFIX_5BYTE_MAPPINGS
  unsigned char *zOut = (unsigned char*)sqlite3_malloc64( nIn*5 + 1 );
#else
  unsigned char *zOut = (unsigned char*)sqlite3_malloc64( nIn*4 + 1 );
#endif
  int c, sz, nOut;
  if( zOut==0 ) return 0;
  nOut = 0;
...
/* Row data for fuzzy-search cursor */
...
static int spellfix1Init(
  ...
  pNew = (spellfix1_vtab*)sqlite3_malloc64( sizeof(*pNew) + nDbName + 1);
  if( pNew==0 ){
    rc = SQLITE_NOMEM;
  }else{
...
static void spellfix1ResizeCursor(spellfix1_cursor *pCur, int N){
  ...
  assert( N>=pCur->nRow );
  aNew = (struct spellfix1_row*)sqlite3_realloc64(pCur->a, sizeof(pCur->a[0])*N);
  if( aNew==0 && N>0 ){
...
static int spellfix1Open(sqlite3_vtab *pVTab, sqlite3_vtab_cursor **ppCursor){
  ...
  pCur = (spellfix1_cursor*)sqlite3_malloc64( sizeof(*pCur) );
...
static int spellfix1FilterForMatch(
  ...
  if( p->zCostTable!=0 && p->pConfig3==0 ){
    p->pConfig3 = (EditDist3Config*)sqlite3_malloc64( sizeof(p->pConfig3[0]) );
