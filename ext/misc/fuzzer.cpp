...
static int fuzzerLoadOneRule(
  ...
  if( iRuleset<0 || iRuleset>FUZZER_MX_RULEID ){
    *pzErr = sqlite3_mprintf("%s: ruleset must be between 0 and %d", 
    ...
    pRule = (fuzzer_rule*)sqlite3_malloc64( sizeof(*pRule) + nFrom + nTo );
    if( pRule==0 ){
      rc = SQLITE_NOMEM;
    }else{
...
static int fuzzerConnect(
  ...
  if( argc!=4 ){
    *pzErr = sqlite3_mprintf(
    ...
    pNew = (fuzzer_vtab*)sqlite3_malloc64( sizeof(*pNew) + nModule + 1);
    if( pNew==0 ){
      rc = SQLITE_NOMEM;
    }else{
...
static int fuzzerOpen(sqlite3_vtab *pVTab, sqlite3_vtab_cursor **ppCursor){
  fuzzer_vtab *p = (fuzzer_vtab*)pVTab;
  fuzzer_cursor *pCur;
  pCur = (fuzzer_cursor*)sqlite3_malloc( sizeof(*pCur) );
...
static int fuzzerRender(
  ...
  if( (*pnBuf)<n+1 ){
    (*pzBuf) = (char*)sqlite3_realloc((*pzBuf), n+100);
...
static fuzzer_stem *fuzzerNewStem(
  ...
  pNew = (fuzzer_stem*)sqlite3_malloc64( sizeof(*pNew) + strlen(zWord) + 1 );
  if( pNew==0 ) return 0;
  memset(pNew, 0, sizeof(*pNew));
