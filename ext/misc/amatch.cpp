...
static int amatchLoadOneRule(
  ...
  {
    pRule = (amatch_rule*)sqlite3_malloc64( sizeof(*pRule) + nFrom + nTo );
    if( pRule==0 ){
      rc = SQLITE_NOMEM;
    }else{
...
static int amatchConnect(
  ...
  (void)pAux;
  *ppVtab = 0;
  pNew = (amatch_vtab*)sqlite3_malloc( sizeof(*pNew) );
...
static int amatchOpen(sqlite3_vtab *pVTab, sqlite3_vtab_cursor **ppCursor){
  amatch_vtab *p = (amatch_vtab*)pVTab;
  amatch_cursor *pCur;
  pCur = (amatch_cursor*)sqlite3_malloc( sizeof(*pCur) );
...
static void amatchAddWord(
  ...
  if( nBase+nTail+3>pCur->nBuf ){
    ...
    pCur->zBuf = (char*)sqlite3_realloc64(pCur->zBuf, pCur->nBuf);
    if( pCur->zBuf==0 ){
      pCur->nBuf = 0;
      return;
    }
  ...
  pWord = (amatch_word*)sqlite3_malloc64( sizeof(*pWord) + nBase + nTail - 1 );
  if( pWord==0 ) return;
...
static int amatchNext(sqlite3_vtab_cursor *cur){
  ...
  do{
    ...
    if( nWord+20>nBuf ){
      nBuf = (char)(nWord+100);
      zBuf = (char*)sqlite3_realloc64(zBuf, nBuf);
