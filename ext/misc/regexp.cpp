...
static int re_resize(ReCompiled *p, unsigned int N){
  ...
  aOp = (char*)sqlite3_realloc64(p->aOp, N*sizeof(p->aOp[0]));
  if( aOp==0 ){ p->zErr = "out of memory"; return 1; }
  p->aOp = aOp;
  aArg = (int*)sqlite3_realloc64(p->aArg, N*sizeof(p->aArg[0]));
...
static const char *re_compile(
  ...
  *ppRe = 0;
  pRe = (ReCompiled*)sqlite3_malloc( sizeof(*pRe) );
  if( pRe==0 ){
...
static void re_sql_func(
  ...
  (void)argc;  /* Unused */
  pRe = (ReCompiled*)sqlite3_get_auxdata(context, 0);
  if( pRe==0 ){
