...
**
** During a write-transaction the fts5_index.cpp module may cache some data 
...
static int fts5SorterNext(Fts5Cursor *pCsr){
  ...
  if( rc==SQLITE_DONE ){
    ...
    aBlob = a = static_cast<const u8 *>(sqlite3_column_blob(pSorter->pStmt, 1));
...
static int fts5NextMethod(sqlite3_vtab_cursor *pCursor){
  ...
  ** clear any token mappings accumulated at the fts5_index.cpp level. In
...
int sqlite3Fts5IsLocaleValue(Fts5Config *pConfig, sqlite3_value *pVal){
  ...
  if( sqlite3_value_type(pVal)==SQLITE_BLOB ){
    ...
    ** be dereferenced.  */
    const u8 *pBlob = static_cast<const u8 *>(sqlite3_value_blob(pVal));
...
int sqlite3Fts5DecodeLocaleValue(
  ...
  const char *p = static_cast<const char *>(sqlite3_value_blob(pVal));
...
static int fts5Init(sqlite3 *db){
  ...
  ** fts5_test_mi.cpp is compiled and linked into the executable. And call
