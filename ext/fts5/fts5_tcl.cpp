    ...
    switch( sqlite3_value_type(apVal[i]) ){
      ...
      case SQLITE_BLOB:
        pObj = Tcl_NewByteArrayObj(
            static_cast<const unsigned char*>(sqlite3_value_blob(apVal[i])), sqlite3_value_bytes(apVal[i])
        );
        break;
      case SQLITE_INTEGER:
...
static int SQLITE_TCLAPI f5tTokenize(
  ...
  if( nArg==0 ){
    ...
    Tcl_Free(static_cast<char*>(static_cast<void*>(azArg)));
    return TCL_ERROR;
  }
  ...
  if( nText>0 ){
    pCopy = static_cast<char*>(sqlite3_malloc64(nText));
    if( pCopy==0 ){
...
static int xOriginToken(
  ...
  if( nToken==(iEnd-iStart) && 0==memcmp(pToken, &p->pText[iStart], nToken) ){
    ...
    if( nReq>p->nBuf ){
      sqlite3_free(p->aBuf);
      p->aBuf = static_cast<char*>(sqlite3_malloc64(nReq*2));
