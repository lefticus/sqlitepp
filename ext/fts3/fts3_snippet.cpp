...
static MatchinfoBuffer *fts3MIBufferNew(size_t nElem, const char *zMatchinfo){
  ...
  pRet = (MatchinfoBuffer*)sqlite3Fts3MallocZero(nByte + nStr+1);
  if( pRet ){
...
static int fts3StringAppend(
  ...
  */
  if( pStr->n+nAppend+1>=pStr->nAlloc ){
    ...
    char *zNew = static_cast<char *>(sqlite3_realloc64(pStr->z, nAlloc));
    if( !zNew ){
      return SQLITE_NOMEM;
    }
    pStr->z = zNew;
...
static int fts3MatchinfoSelectDoctotal(
  ...
  a = static_cast<const char *>(sqlite3_column_blob(pStmt, 0));
  if( a==0 ){
...
static int fts3MatchinfoLcs(Fts3Cursor *pCsr, MatchInfo *pInfo){
  ...
  **/
  aIter = (LcsIterator*)sqlite3Fts3MallocZero(sizeof(LcsIterator) * pCsr->nPhrase);
  ...
  for(i=0; rc==SQLITE_OK && zArg[i]; i++){
    ...
    switch( zArg[i] ){
      ...
      case FTS3_MATCHINFO_LENGTH: {
        ...
        if( rc==SQLITE_OK ){
          int iCol;
          const char *a = static_cast<const char *>(sqlite3_column_blob(pSelectDocsize, 0));
