...
static char *fts3QuoteId(char const *zInput){
  ...
  zRet = static_cast<char *>(sqlite3_malloc64(nRet));
  if( zRet ){
    int i;
    char *z = zRet;
    *(z++) = '"';
...
static int fts3PrefixParameter(
  ...
  aIndex = static_cast<struct Fts3Index *>(sqlite3_malloc64(sizeof(struct Fts3Index) * nIndex));
  *apIndex = aIndex;
  if( !aIndex ){
    return SQLITE_NOMEM;
  }
...
static int fts3DoclistOrMerge(
  ...
  ** order.
  */
  aOut = static_cast<char *>(sqlite3_malloc64((i64)n1+n2+FTS3_VARINT_MAX-1+FTS3_BUFFER_PADDING));
...
static int fts3DoclistPhraseMerge(
  ...
  assert( nDist>0 );
  if( bDescDoclist ){
    aOut = static_cast<char *>(sqlite3_malloc64((sqlite3_int64)*pnRight + FTS3_VARINT_MAX));
...
static int fts3TermSelectMerge(
  ...
  if( pTS->aaOutput[0]==0 ){
    ...
    */
    pTS->aaOutput[0] = static_cast<char *>(sqlite3_malloc64((i64)nDoclist + FTS3_VARINT_MAX + 1));
  ...
  ** calls out here.  */
  ...
  if( iLevel!=FTS3_SEGCURSOR_PENDING ){
    ...
    while( rc==SQLITE_OK && SQLITE_ROW==(rc = sqlite3_step(pStmt)) ){
      ...
      char const *zRoot = static_cast<char const *>(sqlite3_column_blob(pStmt, 4));
...
static int fts3TermSegReaderCursor(
  ...
  pSegcsr = static_cast<Fts3MultiSegReader *>(sqlite3_malloc(sizeof(Fts3MultiSegReader)));
  if( pSegcsr ){
    int i;
...
** implemented in files fts3_tokenizer1.cpp, fts3_porter.cpp and fts3_icu.cpp
...
int sqlite3Fts3Init(sqlite3 *db){
  ...
  pHash = static_cast<Fts3HashWrapper *>(sqlite3_malloc(sizeof(Fts3HashWrapper)));
  if( !pHash ){
    rc = SQLITE_NOMEM;
  }else{
...
** doclists in reverse and by the fts3_write.cpp module to iterate through
  ...
  if( p->nToken==1 ){
    ...
    while( bEof==0 ){
      ...
      if( bEof==0 ){
        ...
        char *aDoclist = static_cast<char *>(sqlite3_malloc64((i64)nByte+FTS3_BUFFER_PADDING));
...
static int fts3EvalNearTest(Fts3Expr *pExpr, int *pRc){
  ...
  ** no exceptions to this - it's the way the parser in fts3_expr.cpp works.
  */
  if( *pRc==SQLITE_OK 
   && pExpr->eType==FTSQUERY_NEAR 
   && (pExpr->pParent==0 || pExpr->pParent->eType!=FTSQUERY_NEAR)
    ...
    aTmp = static_cast<char *>(sqlite3_malloc64(nTmp*2 + FTS3_VARINT_MAX));
    if( !aTmp ){
      *pRc = SQLITE_NOMEM;
      res = 0;
    }else{
