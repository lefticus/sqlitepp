...
** be overridden at runtime for testing purposes. File fts3_test.cpp contains
...
** of type Fts3SegReader* are also used by code in fts3.cpp to iterate through
...
static int fts3PendingListAppendVarint(
  ...
  if( !p ){
    p = static_cast<PendingList *>(sqlite3_malloc64(sizeof(*p) + 100));
    if( !p ){
      return SQLITE_NOMEM;
    }
    p->nSpace = 100;
    ...
    i64 nNew = p->nSpace * 2;
    p = static_cast<PendingList *>(sqlite3_realloc64(p, sizeof(*p) + nNew));
    if( !p ){
      sqlite3_free(*pp);
      *pp = 0;
      return SQLITE_NOMEM;
...
int sqlite3Fts3ReadBlock(
  ...
  if( rc==SQLITE_OK ){
    ...
    *pnBlob = nByte;
    if( paBlob ){
      char *aByte = static_cast<char *>(sqlite3_malloc64((i64)nByte + FTS3_NODE_PADDING));
      if( !aByte ){
        rc = SQLITE_NOMEM;
      }else{
...
static int fts3SegReaderNext(
  ...
  ** overflow - hence the (i64) casts.  */
  if( (i64)nPrefix+nSuffix>(i64)pReader->nTermAlloc ){
    ...
    char *zNew = static_cast<char *>(sqlite3_realloc64(pReader->zTerm, nNew));
    if( !zNew ){
      return SQLITE_NOMEM;
    }
    pReader->zTerm = zNew;
    ...
    if( nReq<=p->nNodeSize || !pTree->zTerm ){
      ...
      if( isCopyTerm ){
        if( pTree->nMalloc<nTerm ){
          char *zNew = static_cast<char *>(sqlite3_realloc64(pTree->zMalloc, (i64)nTerm*2));
          if( !zNew ){
            return SQLITE_NOMEM;
          }
...
static int fts3SegWriterAdd(
  ...
  */
  if( nReq>pWriter->nSize ){
    char *aNew = static_cast<char *>(sqlite3_realloc64(pWriter->aData, nReq));
  ...
  */
  if( isCopyTerm ){
    if( nTerm>pWriter->nMalloc ){
      char *zNew = static_cast<char *>(sqlite3_realloc64(pWriter->zMalloc, (i64)nTerm*2));
      if( !zNew ){
        return SQLITE_NOMEM;
      }
...
static int fts3GrowSegReaderBuffer(Fts3MultiSegReader *pCsr, i64 nReq){
  if( nReq>pCsr->nBuffer ){
    char *aNew;
    pCsr->nBuffer = nReq*2;
    aNew = static_cast<char *>(sqlite3_realloc64(pCsr->aBuffer, pCsr->nBuffer));
    if( !aNew ){
      return SQLITE_NOMEM;
    }
    pCsr->aBuffer = aNew;
...
static void fts3InsertDocsize(
  ...
  if( *pRC ) return;
  pBlob = static_cast<char *>(sqlite3_malloc64( 10*(sqlite3_int64)p->nColumn ));
  if( pBlob==0 ){
    *pRC = SQLITE_NOMEM;
    return;
  }
...
static void fts3UpdateDocTotals(
  ...
  if( *pRC ) return;
  a = static_cast<u32 *>(sqlite3_malloc64( (sizeof(u32)+10)*(sqlite3_int64)nStat ));
  if( a==0 ){
    *pRC = SQLITE_NOMEM;
    return;
  }
  ...
  if( sqlite3_step(pStmt)==SQLITE_ROW ){
    fts3DecodeIntArray(nStat, a,
         static_cast<const char *>(sqlite3_column_blob(pStmt, 0)),
  ...
  if( rc==SQLITE_OK ){
    ...
    for(i=0; rc==SQLITE_OK && sqlite3_step(pStmt)==SQLITE_ROW && i<nSeg; i++){
      rc = sqlite3Fts3SegReaderNew(i, 0,
          ...
          static_cast<const char *>(sqlite3_column_blob(pStmt, 4)),         /* segdir.root */
...
static int fts3IncrmergeLoad(
  ...
  if( rc==SQLITE_OK ){
    ...
    if( sqlite3_step(pSelect)==SQLITE_ROW ){
      ...
      aRoot = static_cast<const char *>(sqlite3_column_blob(pSelect, 4));
      if( aRoot==0 ){
        sqlite3_reset(pSelect);
  ...
  if( rc==SQLITE_OK ){
    ...
    while( SQLITE_ROW==sqlite3_step(pSelect) ){
      if( nIdx>=nAlloc ){
        int *aNew;
        nAlloc += 16;
        aNew = static_cast<int *>(sqlite3_realloc64(aIdx, nAlloc*sizeof(int)));
        if( !aNew ){
          rc = SQLITE_NOMEM;
          break;
        }
...
static int fts3TruncateSegment(
  ...
  if( rc==SQLITE_OK ){
    ...
    if( SQLITE_ROW==sqlite3_step(pFetch) ){
      const char *aRoot = static_cast<const char *>(sqlite3_column_blob(pFetch, 4));
...
static int fts3IncrmergeHintLoad(Fts3Table *p, Blob *pHint){
  ...
  if( rc==SQLITE_OK ){
    ...
    if( SQLITE_ROW==sqlite3_step(pSelect) ){
      const char *aHint = static_cast<const char *>(sqlite3_column_blob(pSelect, 0));
...
int sqlite3Fts3DeferToken(
  ...
  pDeferred = static_cast<Fts3DeferredToken *>(sqlite3_malloc64(sizeof(*pDeferred)));
  if( !pDeferred ){
    return SQLITE_NOMEM;
  }
...
int sqlite3Fts3UpdateMethod(
  ...
  */
  ...
  aSzDel = static_cast<u32 *>(sqlite3_malloc64(sizeof(aSzDel[0])*((sqlite3_int64)p->nColumn+1)*2));
  if( aSzDel==0 ){
    rc = SQLITE_NOMEM;
    goto update_out;
  }
