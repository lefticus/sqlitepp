...
static int getNextString(
  ...
  */
  ...
  if( rc==SQLITE_OK ){
    ...
    for(ii=0; rc==SQLITE_OK; ii++){
      ...
      if( rc==SQLITE_OK ){
        ...
        p = (Fts3Expr*)fts3ReallocOrFree(p, nSpace + ii*sizeof(Fts3PhraseToken));
        zTemp = (char*)fts3ReallocOrFree(zTemp, nTemp + nByte);
        if( !zTemp || !p ){
          rc = SQLITE_NOMEM;
  ...
  if( rc==SQLITE_DONE ){
    ...
    p = (Fts3Expr*)fts3ReallocOrFree(p, nSpace + nToken*sizeof(Fts3PhraseToken) + nTemp);
    if( !p ){
      rc = SQLITE_NOMEM;
      goto getnextstring_out;
    }
    ...
    if( rc==SQLITE_OK ){
      if( p ){
        ...
        if( !sqlite3_fts3_enable_parentheses 
          ...
          Fts3Expr *pNot = (Fts3Expr*)sqlite3Fts3MallocZero(sizeof(Fts3Expr));
          if( !pNot ){
          ...
          */
          ...
          if( isPhrase && !isRequirePhrase ){
            ...
            Fts3Expr *pAnd;
            assert( pRet && pPrev );
            pAnd = (Fts3Expr*)sqlite3Fts3MallocZero(sizeof(Fts3Expr));
            if( !pAnd ){
              sqlite3Fts3ExprFree(p);
              rc = SQLITE_NOMEM;
              goto exprparse_out;
