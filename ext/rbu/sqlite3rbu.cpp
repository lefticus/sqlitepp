...
/*
** These values must match the values defined in wal.cpp for the equivalent
...
static void rbuFossilDeltaFunc(
  ...
  aOut = static_cast<char *>(sqlite3_malloc(nOut+1));
  if( aOut==0 ){
...
static void rbuTargetNameFunc(
  ...
  sqlite3rbu *p = static_cast<sqlite3rbu *>(sqlite3_user_data(pCtx));
  const char *zIn;
...
static void rbuSetupCheckpoint(sqlite3rbu *p, RbuState *pState){
  ...
  */
  ...
  if( p->rc==SQLITE_OK && p->nFrame>0 ){
    ...
    p->aBuf = static_cast<u8 *>(rbuMalloc(p, p->pgsz));
...
static LPWSTR rbuWinUtf8ToUnicode(const char *zFilename){
  ...
  zWideFilename = static_cast<LPWSTR>(sqlite3_malloc64( nChar*sizeof(zWideFilename[0]) ));
  if( zWideFilename==0 ){
    return 0;
  }
...
static sqlite3rbu *rbuMisuseError(void){
  sqlite3rbu *pRet;
  pRet = static_cast<sqlite3rbu *>(sqlite3_malloc64(sizeof(sqlite3rbu)));
  if( pRet ){
