...
static RtreeSearchPoint *rtreeEnqueue(
  ...
  if( pCur->nPoint>=pCur->nPointAlloc ){
    ...
    pNew = static_cast<RtreeSearchPoint *>(sqlite3_realloc64(pCur->aPoint, nNew*sizeof(pCur->aPoint[0])));
    if( pNew==0 ) return 0;
    pCur->aPoint = pNew;
...
static int deserializeGeometry(sqlite3_value *pValue, RtreeConstraint *pCons){
  ...
  pSrc = (RtreeMatchArg*)sqlite3_value_pointer(pValue, "RtreeMatchArg");
  ...
  if( idxNum==1 ){
    ...
    */
    ...
    if( rc==SQLITE_OK && argc>0 ){
      pCsr->aConstraint = static_cast<RtreeConstraint *>(sqlite3_malloc64(sizeof(RtreeConstraint)*argc));
...
static int rtreeBestIndex(sqlite3_vtab *tab, sqlite3_index_info *pIdxInfo){
  ...
  ** possible. */
  ...
  if( iIdx>0 ){
    pIdxInfo->idxStr = static_cast<char *>(sqlite3_malloc( iIdx+1 ));
...
static int SplitNode(
  ...
  */
  aCell = static_cast<RtreeCell *>(sqlite3_malloc64((sizeof(RtreeCell)+sizeof(int))*(nCell+1)));
  if( !aCell ){
    rc = SQLITE_NOMEM;
    goto splitnode_out;
  }
...
static u8 *rtreeCheckGetNode(RtreeCheck *pCheck, i64 iNode, int *pnNode){
  ...
  if( pCheck->rc==SQLITE_OK ){
    ...
    if( sqlite3_step(pCheck->pGetNode)==SQLITE_ROW ){
      ...
      pRet = static_cast<u8 *>(sqlite3_malloc64(nNode));
      if( pRet==0 ){
        pCheck->rc = SQLITE_NOMEM;
      }else{
