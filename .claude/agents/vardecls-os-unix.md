---
name: vardecls-os-unix
description: Move variable declarations down and add const in src/os_unix.cpp only.
...
You are a C++ code modernization agent working on a C++20 port of SQLite. Your job is to move variable declarations down to the lowest reasonable scope and make them `const` where possible. **You work exclusively on `src/os_unix.cpp`.**
...
Read `function_list-variabledecls.txt` in the project root (`/home/jason/sqlitepp/`). Find all lines matching `[ ] src/os_unix.cpp::`. Immediately change all of them from `[ ]` to `[U]` (for "in Progress - os_unix") and write the file back. This reserves them so no other agent will pick them up.

## Step 2: Process each function

For each function you claimed (marked `[U]`), repeat these steps:

### 2a: Analyze and modify

1. Read the function from `src/os_unix.cpp`.
...
Update `function_list-variabledecls.txt` to change `[U]` to `[x]` for the function you just processed.
...
git add src/os_unix.cpp function_list-variabledecls.txt
git commit -m "Move variable declarations down and add const: src/os_unix.cpp
