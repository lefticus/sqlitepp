...
static void kvvfsDecodeJournal(
  ...
  pFile->aJrnl = static_cast<char*>(sqlite3_malloc64( n ));
  if( pFile->aJrnl==0 ){
    pFile->nJrnl = 0;
    return;
  }
...
static int kvvfsReadJrnl(
  ...
  if( pFile->aJrnl==0 ){
    const int szTxt = sqlite3KvvfsMethods.xRcrdRead(pFile->zClass, "jrnl",
                                              0, 0);
    char *aTxt;
    if( szTxt<=4 ){
      return SQLITE_IOERR;
    }
    aTxt = static_cast<char*>(sqlite3_malloc64( szTxt+1 ));
    if( aTxt==0 ) return SQLITE_NOMEM;
    const int rc = sqlite3KvvfsMethods.xRcrdRead(pFile->zClass, "jrnl",
                                       aTxt, szTxt+1);
    if( rc>=0 ){
...
static int kvvfsReadDb(
  ...
  if( got<0 ){
    ...
    if( iOfst+iAmt<512 ){
      ...
      n = kvvfsDecode(aData, static_cast<char*>(zBuf), iAmt);
    }
  }
  if( n<iAmt ){
    memset(static_cast<char*>(zBuf)+n, 0, iAmt-n);
    return SQLITE_IOERR_SHORT_READ;
  }
  return SQLITE_OK;
}
...
static int kvvfsWriteJrnl(
  ...
  KVVfsFile *const pFile = (KVVfsFile*)pProtoFile;
  const sqlite3_int64 iEnd = iOfst+iAmt;
  SQLITE_KV_LOG(("xWrite('%s-journal',%d,%lld)\n", pFile->zClass, iAmt, iOfst));
  if( iEnd>=0x10000000 ) return SQLITE_FULL;
  if( pFile->aJrnl==0 || pFile->nJrnl<iEnd ){
    char *aNew = static_cast<char*>(sqlite3_realloc(pFile->aJrnl, iEnd));
    if( aNew==0 ){
...
static int kvvfsSyncJrnl(sqlite3_file *pProtoFile, int flags){
  ...
  char *const zOut = static_cast<char*>(sqlite3_malloc64( pFile->nJrnl*2 + 50 ));
  if( zOut==0 ){
    return SQLITE_IOERR_NOMEM;
  }
  int n = pFile->nJrnl;
...
static int kvvfsOpen(
  ...
  pFile->aData = static_cast<char*>(sqlite3_malloc64(SQLITE_KVOS_SZ));
  if( pFile->aData==0 ){
    return SQLITE_NOMEM;
  }
  return SQLITE_OK;
...
static int kvvfsCurrentTime(sqlite3_vfs *pVfs, double *pTimeOut){
  sqlite3_int64 i = 0;
  const int rc = kvvfsCurrentTimeInt64(0, &i);
