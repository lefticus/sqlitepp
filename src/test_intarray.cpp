...
static int intarrayCreate(
  ...
  int rc = SQLITE_NOMEM;
  intarray_vtab *pVtab = (intarray_vtab*)sqlite3_malloc64(sizeof(intarray_vtab));
...
SQLITE_API int sqlite3_intarray_create(
  ...
  *ppReturn = p = (sqlite3_intarray*)sqlite3_malloc64( sizeof(*p) );
  if( p==0 ){
    return SQLITE_NOMEM;
  }
  memset(p, 0, sizeof(*p));
...
static int SQLITE_TCLAPI test_intarray_bind(
  ...
  n = objc - 2;
#ifndef SQLITE_OMIT_VIRTUALTABLE
  a = (sqlite3_int64*)sqlite3_malloc64( sizeof(a[0])*n );
  if( a==0 ){
