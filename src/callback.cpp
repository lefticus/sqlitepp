...
static void callCollNeeded(sqlite3 *db, int enc, const char *zName){
  ...
  if( db->xCollNeeded16 ){
    ...
    zExternal = static_cast<const char*>(sqlite3ValueText(pTmp, SQLITE_UTF16NATIVE));
    if( zExternal ){
...
static CollSeq *findCollSeqEntry(
  ...
  CollSeq *pColl;
  pColl = static_cast<CollSeq*>(sqlite3HashFind(&db->aCollSeq, zName));

  if( 0==pColl && create ){
    int nName = sqlite3Strlen30(zName) + 1;
    pColl = static_cast<CollSeq*>(sqlite3DbMallocZero(db, 3*sizeof(*pColl) + nName));
    if( pColl ){
      CollSeq *pDel = 0;
      ...
      pDel = static_cast<CollSeq*>(sqlite3HashInsert(&db->aCollSeq, pColl[0].zName, pColl));
...
FuncDef *sqlite3FindFunction(
  ...
  */
  p = static_cast<FuncDef*>(sqlite3HashFind(&db->aFunc, zName));
  while( p ){
  ...
  */
  if( createFlag && bestScore<FUNC_PERFECT_MATCH && 
      (pBest = static_cast<FuncDef*>(sqlite3DbMallocZero(db, sizeof(*pBest)+nName+1)))!=0 ){
    FuncDef *pOther;
    u8 *z;
    ...
    pOther = static_cast<FuncDef*>(sqlite3HashInsert(&db->aFunc, pBest->zName, pBest));
    if( pOther==pBest ){
...
Schema *sqlite3SchemaGet(sqlite3 *db, Btree *pBt){
  Schema * p;
  if( pBt ){
    p = static_cast<Schema*>(sqlite3BtreeSchema(pBt, sizeof(Schema), sqlite3SchemaClear));
  }else{
    p = static_cast<Schema*>(sqlite3DbMallocZero(0, sizeof(Schema)));
  }
  if( !p ){
    sqlite3OomFault(db);
