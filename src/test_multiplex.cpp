...
struct multiplexGroup {
  struct multiplexReal *aReal;   /* list of all chunks */
...
static int multiplexSubFilename(multiplexGroup *pGroup, int iChunk){
  if( iChunk>=pGroup->nReal ){
    ...
    p = (struct multiplexReal*)sqlite3_realloc64(pGroup->aReal, (iChunk+1)*sizeof(*p));
    if( p==0 ){
      return SQLITE_NOMEM;
    }
...
static sqlite3_file *multiplexSubOpen(
  ...
  *rc = multiplexSubFilename(pGroup, iChunk);
  if( (*rc)==SQLITE_OK && (pSubOpen = pGroup->aReal[iChunk].p)==0 ){
    ...
    pSubOpen = (sqlite3_file*)sqlite3_malloc64( pOrigVfs->szOsFile );
    if( pSubOpen==0 ){
  ...
  */
  ...
  if( rc==SQLITE_OK ){
    ...
    sz = sizeof(multiplexGroup)                             /* multiplexGroup */
       + nName + 1;                                         /* zName */
    pGroup = (multiplexGroup*)sqlite3_malloc64( sz );
    if( pGroup==0 ){
      rc = SQLITE_NOMEM;
    }
  }
...
static int multiplexDelete(
  ...
  if( rc==SQLITE_OK ){
    ...
    */
    ...
    char *z;
    z = (char*)sqlite3_malloc64(nName + 5);
    if( z==0 ){
      rc = SQLITE_IOERR_NOMEM;
    }else{
      int iChunk = 0;
