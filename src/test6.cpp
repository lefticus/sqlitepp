...
static void *crash_realloc(void *p, int n){
  return (void *)Tcl_AttemptRealloc((char*)p, (size_t)n);
}
    ...
    */
    ...
    switch( eAction ){
      ...
      case 3: {               /* Trash sectors */
        ...
        zGarbage = (u8*)crash_malloc(g.iSectorSize);
        if( zGarbage ){
          sqlite3_int64 i;
...
static int cfWrite(
  ...
  while( pCrash->iSize>pCrash->nData ){
    ...
    zNew = (u8*)crash_realloc(pCrash->zData, nNew);
    if( !zNew ){
      return SQLITE_NOMEM;
    }
  ...
  return writeListAppend(pFile, iOfst, (const u8*)zBuf, iAmt);
}
...
static int cfOpen(
  ...
  if( rc==SQLITE_OK ){
    ...
    pWrapper->zData = (u8*)crash_malloc(pWrapper->nData);
    if( pWrapper->zData ){
