...
static struct vxworksFileId *vxworksFindFileId(const char *zAbsoluteName){
  ...
  pNew = static_cast<struct vxworksFileId *>(sqlite3_malloc64( sizeof(*pNew) + (n+1) ));
  if( pNew==0 ) return 0;
...
static int findInodeInfo(
  ...
  */
  ...
  if( pInode==0 ){
    pInode = static_cast<unixInodeInfo *>(sqlite3_malloc64( sizeof(*pInode) ));
    if( pInode==0 ){
...
static int unixFileControl(sqlite3_file *id, int op, void *pArg){
  ...
  switch( op ){
    ...
    case SQLITE_FCNTL_TEMPFILENAME: {
      char *zTFile = static_cast<char *>(sqlite3_malloc64( pFile->pVfs->mxPathname ));
      if( zTFile ){
...
static int unixOpenSharedMemory(unixFile *pDbFd){
  ...
  p = static_cast<struct unixShm *>(sqlite3_malloc64( sizeof(*p) ));
...
#endif
    pShmNode = static_cast<struct unixShmNode *>(sqlite3_malloc64( sizeof(*pShmNode) + nShmFilename ));
    if( pShmNode==0 ){
...
static int unixShmMap(
  ...
  if( pShmNode->nRegion<nReqRegion ){
    ...
    apNew = static_cast<char**>(sqlite3_realloc64(
...
static void unixRemapfile(
...
#if HAVE_MREMAP
    pNew = static_cast<u8*>(osMremap(pOrig, nReuse, nNew, MREMAP_MAYMOVE));
    zErr = "mremap";
#else
    pNew = static_cast<u8*>(osMmap(pReq, nNew-nReuse, flags, MAP_SHARED, h, nReuse));
    if( pNew!=MAP_FAILED ){
      if( pNew!=pReq ){
  ...
  if( pNew==0 ){
    pNew = static_cast<u8*>(osMmap(0, nNew, flags, MAP_SHARED, h, 0));
  }
...
static int fillInUnixFile(
  ...
  if( pLockingStyle == &posixIoMethods
    ...
    */
    ...
    pNew->lockingContext = pCtx = static_cast<afpLockingContext *>(sqlite3_malloc64( sizeof(*pCtx) ));
    if( pCtx==0 ){
      rc = SQLITE_NOMEM_BKPT;
    }else{
    ...
    */
    ...
    zLockFile = static_cast<char*>(sqlite3_malloc64(nFilename));
    if( zLockFile==0 ){
      rc = SQLITE_NOMEM_BKPT;
    }else{
  ...
  */
  ...
  if( eType==SQLITE_OPEN_MAIN_DB ){
    ...
    if( pUnused ){
      ...
      pUnused = static_cast<UnixUnusedFd *>(sqlite3_malloc64(sizeof(*pUnused)));
      if( !pUnused ){
        return SQLITE_NOMEM_BKPT;
      }
    }
...
static int proxyCreateUnixFile(
  ...
  */
  ...
  pNew = static_cast<unixFile*>(sqlite3_malloc64(sizeof(*pNew)));
  if( pNew==NULL ){
    rc = SQLITE_NOMEM_BKPT;
...
static int proxyCreateConchPathname(char *dbPath, char **pConchPath){
  ...
  *pConchPath = conchPath = static_cast<char*>(sqlite3_malloc64(len + 8));
  if( conchPath==0 ){
    return SQLITE_NOMEM_BKPT;
  }
...
static int proxyTransformUnixFile(unixFile *pFile, const char *path) {
  ...
  pCtx = static_cast<proxyLockingContext *>(sqlite3_malloc64( sizeof(*pCtx) ));
  if( pCtx==0 ){
    return SQLITE_NOMEM_BKPT;
  }
