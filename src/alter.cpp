...
** the copy. The copy of the Table structure is deleted by tokenize.cpp
...
const void *sqlite3RenameTokenMap(
  ...
  if( ALWAYS(pParse->eParseMode!=PARSE_MODE_UNMAP) ){
    pNew = static_cast<RenameToken *>(sqlite3DbMallocZero(pParse->db, sizeof(RenameToken)));
    if( pNew ){
      pNew->p = pPtr;
      pNew->t = *pToken;
...
static int renameEditSql(
  ...
  if( zNew ){
    ...
    ** quoted version of the new column name.  */
    ...
    zOut = static_cast<char *>(sqlite3DbMallocZero(db, (u64)nSql + pRename->nList*(u64)nQuot + 1));
  }else{
    assert( nSql>0 );
...
static int quotedCompare(
  ...
  zCopy = static_cast<char *>(sqlite3MallocZero(nQuote+1));
  if( zCopy==0 ){
