...
** it is needed by "mutex_w32.cpp" (when debugging), "os_win.cpp" (when
...
** callback is currently invoked only from within pager.cpp.
*/
...
struct sqlite3 {
  ...
  ** mutex, not by sqlite3.mutex. They are used by code in notify.cpp.
  **
...
#define SQLITE_AllOpts        (int)0xffffffff /* All optimizations */
...
#define MFUNCTION(zName, nArg, xPtr, xFunc) \
  {nArg, SQLITE_FUNC_BUILTIN|SQLITE_FUNC_CONSTANT|SQLITE_UTF8, \
   reinterpret_cast<void*>(xPtr), 0, xFunc, 0, 0, 0, #zName, {0} }
...
** analyze.cpp source file for additional information.
*/
struct IndexSample {
...
struct AggInfo {
  ...
  int iFirstReg;          /* First in range for aCol[] and aFunc[] */
...
/*
** Macros to compute aCol[] and aFunc[] numbers.
**
...
** number containing the result of the subquery.  If the
...
struct Expr {
  ...
  *********************************************************************/
  ...
  int iTable;            /* TK_COLUMN: cursor number of table holding column
                         ...
                         ** TK_SELECT: 1st of result vector */
...
struct ExprList {
  ...
  struct ExprList_item a[FLEXARRAY]; /* One slot for each expression in the list */
};
...
struct IdList {
  ...
  struct IdList_item a[FLEXARRAY];
};
...
struct Upsert {
  ...
  ** statement and will be freed by INSERT processing. */
  ...
  int regData;              /* First holding array of VALUES */
...
**                     in pDest->iSDParm then abandon the rest
...
**                     a holding a Bloom filter for the IN operator
...
**                     of the co-routine is stored in pDest->iSDParm
...
struct SelectDest {
  ...
  int iSdst;           /* Base where results are written */
...
struct AutoincInfo {
  ...
  int regCtr;           /* Memory holding the rowid counter */
};
...
struct Parse {
  ...
  int nRangeReg;       /* Size of the temporary block */
  int iRangeReg;       /* First in temporary block */
  int nErr;            /* Number of errors seen */
  int nTab;            /* Number of previously allocated VDBE cursors */
  int nMem;            /* Number of memory cells used so far */
  int szOpAlloc;       /* Bytes of memory space allocated for Vdbe.aOp[] */
  int iSelfTab;        /* Table associated with an index on expr, or negative
                       ** of the base during check-constraint eval */
...
#define OPFLAG_P2ISREG       0x10    /* P2 to OP_Open** is a number */
...
struct CteUse {
  ...
  int regRtn;            /* Return address for addrM9e subroutine */
...
struct Window {
  ...
  int regApp;             /* Function (also used by min/max) */
...
** be defined locally, but now we use the varint routines in the util.cpp
** file.
*/
...
/* Declarations for functions in fkey.cpp. All of these are replaced by
...
/*
** The interface to the code in fault.cpp used for identifying "benign"
...
/*
** These routines are available for the mem2.cpp debugging memory allocator
