...
static int memjrnlWrite(
  ...
  MemJournal *p = (MemJournal *)pJfd;
  int nWrite = iAmt;
  const u8 *zWrite = static_cast<const u8*>(zBuf);
    ...
    if( iOfst==0 && p->pFirst ){
      ...
      while( nWrite>0 ){
        ...
        if( iChunkOffset==0 ){
          ...
          FileChunk *pNew = static_cast<FileChunk*>(sqlite3_malloc(fileChunkSize(p->nChunkSize)));
          if( !pNew ){
...
*/
constexpr inline struct sqlite3_io_methods MemJournalMethods = {
  1,                /* iVersion */
  memjrnlClose,     /* xClose */
  memjrnlRead,      /* xRead */
  memjrnlWrite,     /* xWrite */
