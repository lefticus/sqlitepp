...
**
** This file was originally part of where.cpp but was split out to improve
...
static int whereClauseInsert(WhereClause *pWC, Expr *p, u16 wtFlags){
  ...
  if( pWC->nTerm>=pWC->nSlot ){
    ...
    pWC->a = static_cast<WhereTerm*>(sqlite3WhereMalloc(pWC->pWInfo, sizeof(pWC->a[0])*pWC->nSlot*2 ));
    if( pWC->a==0 ){
...
static void exprAnalyzeOrTerm(
  ...
  */
  ...
  pTerm->u.pOrInfo = pOrInfo = static_cast<WhereOrInfo*>(sqlite3DbMallocZero(db, sizeof(*pOrInfo)));
  if( pOrInfo==0 ) return;
  ...
  */
  ...
  for(i=pOrWc->nTerm-1, pOrTerm=pOrWc->a; i>=0 && indexable; i--, pOrTerm++){
    if( (pOrTerm->eOperator & WO_SINGLE)==0 ){
      ...
      chngToIN = 0;
      pAndInfo = static_cast<WhereAndInfo*>(sqlite3DbMallocRawNN(db, sizeof(*pAndInfo)));
      if( pAndInfo ){
        WhereClause *pAndWC;
        WhereTerm *pAndTerm;
        int j;
...
/***************************************************************************
** Routines with file scope above.  Interface to the rest of the where.cpp
** subsystem follows.
***************************************************************************/
...
**       complex expressions that require pushing values onto
