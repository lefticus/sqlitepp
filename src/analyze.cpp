  ...
  for(i=0; i<static_cast<int>(aTable.size()); i++){
    ...
    if( (pStat = sqlite3FindTable(db, zTab, pDb->zDbSName))==0 ){
      if( i<nToOpen ){
        ...
        ** of the new table in pParse->regRoot. This is important 
...
static void sampleSetRowid(sqlite3 *db, StatSample *p, int n, const u8 *pData){
  ...
  p->u.aRowid = static_cast<u8 *>(sqlite3DbMallocRawNN(db, n));
  if( p->u.aRowid ){
    p->nRowid = n;
...
#endif
  p = static_cast<StatAccum *>(sqlite3DbMallocZero(db, n));
  if( p==0 ){
...
static void statPush(
  ...
  StatAccum *p = static_cast<StatAccum*>(const_cast<void*>(sqlite3_value_blob(argv[0])));
  ...
  if( p->mxSample ){
    ...
    if( sqlite3_value_type(argv[2])==SQLITE_INTEGER ){
      ...
      sampleSetRowid(p->db, &p->current, sqlite3_value_bytes(argv[2]),
                                         static_cast<const u8 *>(sqlite3_value_blob(argv[2])));
    }
...
static void analyzeOneTable(
  ...
  int regTemp = iMem++;        /* Temporary use */
  int regTemp2 = iMem++;       /* Second temporary use */
  ...
  ** this time though.  */
  ...
  for(pIdx=pTab->pIndex; pIdx; pIdx=pIdx->pNext){
    ...
    /* Populate the containing the index name. */
    ...
    **    (3) estimated number of rows in the index. */
    ...
    if( nColTest>0 ){
      ...
      aGotoChng = static_cast<int *>(sqlite3DbMallocRawNN(db, sizeof(int)*nColTest));
      ...
      */
      ...
      for(i=0; i<nColTest; i++){
        char *pColl = reinterpret_cast<char*>(sqlite3LocateCollSeq(pParse, pIdx->azColl[i]));
  ...
  if( pIndex ){
    ...
    ** the old data with the new instead of allocating a new array.  */
    if( pIndex->aiRowEst==0 ){
      pIndex->aiRowEst = static_cast<tRowcnt*>(sqlite3MallocZero(sizeof(tRowcnt) * nCol));
...
static int loadStatTbl(
  ...
  while( sqlite3_step(pStmt)==SQLITE_ROW ){
    ...
    pIdx->aSample = static_cast<IndexSample *>(sqlite3DbMallocZero(db, nByte));
    if( pIdx->aSample==0 ){
...
int sqlite3AnalysisLoad(sqlite3 *db, int iDb){
  ...
  for(i=sqliteHashFirst(&pSchema->idxHash); i; i=sqliteHashNext(i)){
    Index *pIdx = static_cast<Index*>(sqliteHashData(i));
    pIdx->hasStat1 = 0;
#ifdef SQLITE_ENABLE_STAT4
