...
void sqlite3Update(
  ...
  int eOnePass;          /* ONEPASS_XXX value from where.cpp */
...
# define isView 0
  ...
  */
  aXRef = static_cast<int*>(sqlite3DbMallocRawNN(db, sizeof(int) * (pTab->nCol+nIdx+1) + nIdx+2 ));
  ...
  ** being updated.  Fill in aRegIdx[] with a number that will hold
  ...
  */
  ...
  if( !IsVirtual(pTab) ){
    ...
    ** reallocated.  aRegIdx[nAllIdx] is the in which the main
  ...
  ** initialize the count of updated rows */
  ...
  /* If the rowid value will change, set regNewRowid to
  ** contain the new value. If the rowid is not being modified,
  ** then regNewRowid is the same as regOldRowid, which is
  ** already populated.  */
...
** stores the same values (A, B and C above) in a array and
...
static void updateVirtualTable(
  ...
  int regArg;                     /* First in VUpdate arg array */
  ...
  if( pSrc->nSrc>1 ){
    ...
    if( eOnePass ){
      ...
      ** above. */
      ...
      /* Create a record from the argument contents and insert it into
      ** the ephemeral table. */
      sqlite3MultiWrite(pParse);
