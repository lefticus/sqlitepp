...
Vdbe *sqlite3VdbeCreate(Parse *pParse){
  ...
  Vdbe *p = static_cast<Vdbe*>(sqlite3DbMallocRawNN(db, sizeof(Vdbe) ));
  if( p==0 ) return 0;
...
void sqlite3VdbeAddDblquoteStr(sqlite3 *db, Vdbe *p, const char *z){
  if( p ){
    ...
    DblquoteStr *pStr = static_cast<DblquoteStr*>(sqlite3DbMallocRawNN(db,
...
static int growOpArray(Vdbe *v, int nOp){
  ...
  ** size of the op array or add 1KB of space, whichever is smaller. */
  ...
  VdbeOp *pNew = static_cast<VdbeOp*>(sqlite3DbRealloc(p->db, v->aOp, nNew*sizeof(Op)));
  if( pNew ){
...
** One is initialized for each characgter in zTypes[].  For each
** "s" character in zTypes[], the is a string if the argument is
** not NULL, or OP_Null if the value is a null pointer.  For each "i" character
** in zTypes[], the is initialized to an integer.
**
...
int sqlite3VdbeAddOp4(
  ...
  const int addr = sqlite3VdbeAddOp3(p, op, p1, p2, p3);
...
int sqlite3VdbeAddFunctionCall(
  ...
  assert( v );
  pCtx = static_cast<sqlite3_context*>(sqlite3DbMallocRawNN(pParse->db, SZ_CONTEXT(nArg)));
  if( pCtx==0 ){
  ...
  const int addr = sqlite3VdbeAddOp4(v, eCallCtx ? OP_PureFunc : OP_Function,
...
int sqlite3VdbeAddOp4Dup8(
  ...
  char *p4copy = static_cast<char*>(sqlite3DbMallocRawNN(sqlite3VdbeDb(p), 8));
...
int sqlite3VdbeExplainParent(Parse *pParse){
  ...
  const VdbeOp *pOp = sqlite3VdbeGetOp(pParse->pVdbe, pParse->addrExplain);
  return pOp->p2;
}
...
int sqlite3VdbeExplain(Parse *pParse, u8 bPush, const char *zFmt, ...){
  ...
  ** But omit them (for performance) during production builds */
...
#endif
  {
    va_list ap;
    va_start(ap, zFmt);
    char *const zMsg = sqlite3VMPrintf(pParse->db, zFmt, ap);
    va_end(ap);
    Vdbe *const v = pParse->pVdbe;
    const int iThis = v->nOp;
...
void sqlite3VdbeAddParseSchemaOp(Vdbe *p, int iDb, char *zWhere, u16 p5){
  ...
  for(int j=0; j<p->db->nDb; j++) sqlite3VdbeUsesBtree(p, j);
...
void sqlite3VdbeEndCoroutine(Vdbe *v, int regYield){
  ...
  /* Clear the temporary cache, thereby ensuring that each
...
static SQLITE_NOINLINE void resizeResolveLabel(Parse *p, Vdbe *v, int j){
  const int nNewSize = 10 - p->nLabel;
  p->aLabel = static_cast<int*>(sqlite3DbReallocOrFree(p->db, p->aLabel,
...
static Op *opIterNext(VdbeOpIter *p){
  ...
  if( p->iSub<=p->nSub ){
    ...
    if( pRet->p4type==P4_SUBPROGRAM ){
      ...
      if( j==p->nSub ){
        p->apSub = static_cast<SubProgram**>(sqlite3DbReallocOrFree(v->db, p->apSub, nByte));
        if( !p->apSub ){
          pRet = 0;
        }else{
...
static void resolveP2Values(Vdbe *p, int *pMaxVtabArgs){
  ...
  Op *pOp;
  Parse *const pParse = p->pParse;
  int *const aLabel = pParse->aLabel;
...
void sqlite3VdbeNoJumpsOutsideSubrtn(
  ...
  int iRetReg       /* Subroutine return address */
){
  sqlite3_str *pErr = 0;
  assert( v!=0 );
  Parse *const pParse = v->pParse;
  assert( pParse!=0 );
  if( pParse->nErr ) return;
  assert( iLast>=iFirst );
  assert( iLast<v->nOp );
  VdbeOp *pOp = &v->aOp[iFirst];
  for(int i=iFirst; i<=iLast; i++, pOp++){
...
** by code in pragma.cpp to ensure that the implementation of certain
...
void sqlite3VdbeScanStatus(
  ...
  if( IS_STMT_SCANSTATUS(p->db) ){
    ...
    ScanStatus *aNew;
    aNew = static_cast<ScanStatus*>(sqlite3DbRealloc(p->db, p->aScan, nByte));
    if( aNew ){
...
** into iDest, then add the OPFLAG_TYPEOFARG flag to that
** opcode.
*/
...
void sqlite3VdbeReleaseRegisters(
  Parse *pParse,       /* Parsing context */
  int iFirst,          /* Index of first to be released */
...
void sqlite3VdbeAppendP4(Vdbe *p, void *pP4, int n){
  ...
  if( p->db->mallocFailed ){
    ...
    assert( p->nOp>0 );
    VdbeOp *const pOp = &p->aOp[p->nOp-1];
...
void sqlite3VdbeSetP4KeyInfo(Parse *pParse, Index *pIdx){
  ...
  assert( v!=0 );
  assert( pIdx!=0 );
  KeyInfo *const pKeyInfo = sqlite3KeyInfoOfIndex(pParse, pIdx);
...
**
** The Synopsis: field in comments in the vdbe.cpp source file gets converted
...
void sqlite3VdbePrintOp(FILE *pOut, int pc, VdbeOp *pOp){
  ...
  ** information from the vdbe.cpp source text */
...
void sqlite3VdbeFrameDelete(VdbeFrame *p){
  Mem *const aMem = VdbeFrameMem(p);
  VdbeCursor **const apCsr = (VdbeCursor **)&aMem[p->nChildMem];
  assert( sqlite3VdbeFrameIsValid(p) );
  for(int i=0; i<p->nChildCsr; i++){
...
void sqlite3VdbeMakeReady(
  ...
  */
  x.nNeeded = 0;
  p->aMem = static_cast<Mem*>(allocSpace(&x, 0, nMem*sizeof(Mem)));
  p->aVar = static_cast<Mem*>(allocSpace(&x, 0, nVar*sizeof(Mem)));
  p->apArg = static_cast<Mem**>(allocSpace(&x, 0, nArg*sizeof(Mem*)));
  p->apCsr = static_cast<VdbeCursor**>(allocSpace(&x, 0, nCursor*sizeof(VdbeCursor*)));
  if( x.nNeeded ){
    p->pFree = sqlite3DbMallocRawNN(db, x.nNeeded);
    x.pSpace = static_cast<u8*>(p->pFree);
    x.nFree = x.nNeeded;
    if( !db->mallocFailed ){
      p->aMem = static_cast<Mem*>(allocSpace(&x, p->aMem, nMem*sizeof(Mem)));
      p->aVar = static_cast<Mem*>(allocSpace(&x, p->aVar, nVar*sizeof(Mem)));
      p->apArg = static_cast<Mem**>(allocSpace(&x, p->apArg, nArg*sizeof(Mem*)));
      p->apCsr = static_cast<VdbeCursor**>(allocSpace(&x, p->apCsr, nCursor*sizeof(VdbeCursor*)));
    }
  }
#ifdef SQLITE_DEBUG
  p->napArg = nArg;
...
void sqlite3VdbeSetNumCols(Vdbe *p, int nResColumn){
  ...
  const int n = nResColumn*COLNAME_N;
  p->nResColumn = p->nResAlloc = (u16)nResColumn;
  p->aColName = static_cast<Mem*>(sqlite3DbMallocRawNN(db, sizeof(Mem)*n ));
...
int sqlite3VdbeSetColName(
  ...
  assert( p->aColName!=0 );
  Mem *const pColName = &(p->aColName[idx+var*p->nResAlloc]);
  const int rc = sqlite3VdbeMemSetText(pColName, zName, -1, xDel);
  ...
  ** SQL statement does not read or write a database file.  */
  if( p->bIsReader ){
    ...
    if( isSpecialError ){
      ...
      ** pagerStress() in pager.cpp), the rollback is required to restore
...
int sqlite3VdbeReset(Vdbe *p){
  ...
  */
  ...
  /* Reset contents and reclaim error message memory.
  */
#ifdef SQLITE_DEBUG
...
UnpackedRecord *sqlite3VdbeAllocUnpackedRecord(
  ...
  p = static_cast<UnpackedRecord*>(sqlite3DbMallocRaw(pKeyInfo->db, nByte));
  if( !p ) return 0;
...
SQLITE_NOINLINE int sqlite3BlobCompare(const Mem *pB1, const Mem *pB2){
  ...
  ** sqlite3MemCompare(). */
  ...
  const int c = memcmp(pB1->z, pB2->z, n1>n2 ? n2 : n1);
  if( c ) return c;
  return n1 - n2;
}
...
int sqlite3MemCompare(const Mem *pMem1, const Mem *pMem2, const CollSeq *pColl){
  const int f1 = pMem1->flags;
  const int f2 = pMem2->flags;
  const int combined_flags = f1|f2;
...
int sqlite3VdbeIdxKeyCompare(
  ...
  BtCursor *const pCur = pC->uc.pCursor;
  assert( sqlite3BtreeCursorIsValid(pCur) );
  nCellKey = sqlite3BtreePayloadSize(pCur);
...
static int vdbeIsMatchingIndexKey(
  ...
  ** if the record is corrupt. */
  aRec = static_cast<u8*>(sqlite3MallocZero(nRec+5));
  if( aRec==0 ){
...
#endif
  const VdbeOp *const pOp = pCtx->pVdbe->aOp + pCtx->iOp;
...
** byte-code values correctly initialized.
*/
...
** the vdbeUnpackRecord() function found in vdbeapi.cpp.
*/
