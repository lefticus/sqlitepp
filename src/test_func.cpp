...
extern void sqlite3BeginBenignMalloc(void);
extern void sqlite3EndBenignMalloc(void);
#else
...
static void test_auxdata(
  ...
  int i;
  char *zRet = (char*)testContextMalloc(pCtx, nArg*2);
  if( !zRet ) return;
  ...
  for(i=0; i<nArg; i++){
    ...
    if( z ){
      int n;
      char *zAux = (char*)sqlite3_get_auxdata(pCtx, i);
      if( zAux ){
        zRet[i*2] = '1';
      ...
      n = (int)strlen(z) + 1;
      zAux = (char*)testContextMalloc(pCtx, n);
      if( zAux ){
        memcpy(zAux, z, n);
