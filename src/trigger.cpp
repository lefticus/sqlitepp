...
void sqlite3BeginTrigger(
  ...
  */
  ...
  pTrigger = static_cast<Trigger*>(sqlite3DbMallocZero(db, sizeof(Trigger)));
...
void sqlite3FinishTrigger(
  ...
  */
  ...
  if( db->init.busy ){
    ...
    assert( sqlite3SchemaMutexHeld(db, iDb, 0) );
    assert( pLink!=0 );
    pTrig = static_cast<Trigger*>(sqlite3HashInsert(pHash, zName, pTrig));
    if( pTrig ){
      sqlite3OomFault(db);
    }else if( pLink->pSchema==pLink->pTabSchema ){
      Table *pTab;
      pTab = static_cast<Table*>(sqlite3HashFind(&pLink->pTabSchema->tblHash, pLink->table));
      assert( pTab!=0 );
...
TriggerStep *sqlite3TriggerSelectStep(
  ...
  TriggerStep *pTriggerStep = static_cast<TriggerStep*>(sqlite3DbMallocZero(db, sizeof(TriggerStep)));
  if( pTriggerStep==0 ) {
  ...
  if( pParse->nErr==0 ){
    if( pNew 
     ...
     && pTabList->a[0].u4.zDatabase 
      ...
      pTriggerStep = static_cast<TriggerStep*>(sqlite3DbMallocZero(db, sizeof(TriggerStep)));
      if( pTriggerStep ){
...
static Table *tableOfTrigger(Trigger *pTrigger){
  return static_cast<Table*>(sqlite3HashFind(&pTrigger->pTabSchema->tblHash, pTrigger->table));
}
...
static TriggerPrg *codeRowTrigger(
  ...
  ** list of the top-level Parse object sooner rather than later.  */
  pPrg = static_cast<TriggerPrg*>(sqlite3DbMallocZero(db, sizeof(TriggerPrg)));
  if( !pPrg ) return 0;
  pPrg->pNext = pTop->pTriggerPrg;
  pTop->pTriggerPrg = pPrg;
  pPrg->pProgram = pProgram = static_cast<SubProgram*>(sqlite3DbMallocZero(db, sizeof(SubProgram)));
  ...
  ** trigger sub-program.  */
  ...
  if( v ){
    ...
    ** OP_Halt inserted at the end of the program.  */
    ...
    pProgram->token = static_cast<void*>(pTrigger);
