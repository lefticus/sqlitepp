...
static SQLITE_NOINLINE void invokeProfileCallback(sqlite3 *db, Vdbe *p){
  ...
  const sqlite3_int64 iElapse = (iNow - p->startTime)*1000000;
#ifndef SQLITE_OMIT_DEPRECATED
  if( db->xProfile ){
...
int sqlite3_clear_bindings(sqlite3_stmt *pStmt){
  ...
  sqlite3_mutex * const mutex = p->db->mutex;
#endif
...
const unsigned char *sqlite3_value_text(sqlite3_value *pVal){
  return static_cast<const unsigned char*>(sqlite3ValueText(pVal, SQLITE_UTF8));
}
...
sqlite3_value *sqlite3_value_dup(const sqlite3_value *pOrig){
  sqlite3_value *pNew;
  if( pOrig==0 ) return 0;
  pNew = static_cast<sqlite3_value*>(sqlite3_malloc( sizeof(*pNew) ));
  if( pNew==0 ) return 0;
...
void sqlite3_result_blob(
  ...
  setResultStrOrError(pCtx, static_cast<const char*>(z), n, 0, xDel);
}
void sqlite3_result_blob64(
  ...
  if( n>0x7fffffff ){
    ...
    setResultStrOrError(pCtx, static_cast<const char*>(z), (int)n, 0, xDel);
  }
}
...
void sqlite3_result_error16(sqlite3_context *pCtx, const void *z, int n){
  ...
  assert( sqlite3_mutex_held(pCtx->pOut->db->mutex) );
  pCtx->isError = SQLITE_ERROR;
  sqlite3VdbeMemSetStr(pCtx->pOut, static_cast<const char*>(z), n, SQLITE_UTF16NATIVE, SQLITE_TRANSIENT);
}
#endif
...
void sqlite3_result_text16(
  ...
  assert( sqlite3_mutex_held(pCtx->pOut->db->mutex) );
  setResultStrOrError(pCtx, static_cast<const char*>(z), n & ~(u64)1, SQLITE_UTF16NATIVE, xDel);
}
void sqlite3_result_text16be(
  ...
  assert( sqlite3_mutex_held(pCtx->pOut->db->mutex) );
  setResultStrOrError(pCtx, static_cast<const char*>(z), n & ~(u64)1, SQLITE_UTF16BE, xDel);
}
void sqlite3_result_text16le(
  ...
  assert( sqlite3_mutex_held(pCtx->pOut->db->mutex) );
  setResultStrOrError(pCtx, static_cast<const char*>(z), n & ~(u64)1, SQLITE_UTF16LE, xDel);
}
#endif /* SQLITE_OMIT_UTF16 */
...
static int valueFromValueList(
  ...
  *ppOut = 0;
  ...
  ValueList * const pRhs = (ValueList*)pVal->z;
  int rc;
  if( bNext ){
...
#endif
  if( *piTime==0 ){
    const int rc = sqlite3OsCurrentTimeInt64(p->pOut->db->pVfs, piTime);
    if( rc ) *piTime = 0;
  }
  return *piTime;
}
...
void *sqlite3_get_auxdata(sqlite3_context *pCtx, int iArg){
  ...
  assert( pCtx->pVdbe!=0 );
#endif
  for(AuxData *pAuxData=pCtx->pVdbe->pAuxData; pAuxData; pAuxData=pAuxData->pNextAux){
...
void sqlite3_set_auxdata(
  ...
  if( pAuxData==0 ){
    pAuxData = static_cast<AuxData*>(sqlite3DbMallocZero(pVdbe->db, sizeof(AuxData)));
...
const void *sqlite3_column_blob(sqlite3_stmt *pStmt, int i){
  const void * const val = sqlite3_value_blob( columnMem(pStmt,i) );
...
const char *sqlite3_column_name(sqlite3_stmt *pStmt, int N){
  return static_cast<const char*>(columnName(pStmt, N, 0, COLNAME_NAME));
}
#ifndef SQLITE_OMIT_UTF16
...
const char *sqlite3_column_decltype(sqlite3_stmt *pStmt, int N){
  return static_cast<const char*>(columnName(pStmt, N, 0, COLNAME_DECLTYPE));
}
#ifndef SQLITE_OMIT_UTF16
...
const char *sqlite3_column_database_name(sqlite3_stmt *pStmt, int N){
  return static_cast<const char*>(columnName(pStmt, N, 0, COLNAME_DATABASE));
}
#ifndef SQLITE_OMIT_UTF16
...
const char *sqlite3_column_table_name(sqlite3_stmt *pStmt, int N){
  return static_cast<const char*>(columnName(pStmt, N, 0, COLNAME_TABLE));
}
#ifndef SQLITE_OMIT_UTF16
...
const char *sqlite3_column_origin_name(sqlite3_stmt *pStmt, int N){
  return static_cast<const char*>(columnName(pStmt, N, 0, COLNAME_COLUMN));
}
#ifndef SQLITE_OMIT_UTF16
...
static int bindText(
  ...
  Vdbe * const p = (Vdbe *)pStmt;
  int rc = vdbeUnbind(p, (u32)(i-1));
  if( rc==SQLITE_OK ){
    assert( p!=0 && p->aVar!=0 && i>0 && i<=p->nVar ); /* tag-20240917-01 */
    if( zData!=0 ){
      Mem * const pVar = &p->aVar[i-1];
      if( encoding==SQLITE_UTF8 ){
        rc = sqlite3VdbeMemSetText(pVar, static_cast<const char*>(zData), nData, xDel);
        ...
        ** that are documented to result in undefined behavior. */
        ...
        rc = sqlite3VdbeMemSetStr(pVar, static_cast<const char*>(zData), nData, encoding, xDel);
...
int sqlite3TransferBindings(sqlite3_stmt *pFromStmt, sqlite3_stmt *pToStmt){
  Vdbe * const pFrom = (Vdbe*)pFromStmt;
  Vdbe * const pTo = (Vdbe*)pToStmt;
  assert( pTo->db==pFrom->db );
  assert( pTo->nVar==pFrom->nVar );
  sqlite3_mutex_enter(pTo->db->mutex);
  for(int i=0; i<pFrom->nVar; i++){
...
#else
  char *z = 0;
  const char * const zSql = sqlite3_sql(pStmt);
  if( zSql ){
    Vdbe *p = (Vdbe *)pStmt;
    sqlite3_mutex_enter(p->db->mutex);
...
static UnpackedRecord *vdbeUnpackRecord(
  ...
  UnpackedRecord * const pRet = sqlite3VdbeAllocUnpackedRecord(pKeyInfo);
  if( pRet ){
...
#endif
  PreUpdate * const p = db->pPreUpdate;
  ...
  if( iIdx==p->pTab->iPKey ){
    *ppValue = pMem = &p->oldipk;
    ...
    if( p->pUnpacked==0 ){
      ...
      aRec = static_cast<u8*>(sqlite3DbMallocRaw(db, nRec));
...
#ifdef SQLITE_ENABLE_API_ARMOR
  PreUpdate * const p = db!=0 ? db->pPreUpdate : 0;
#else
  PreUpdate * const p = db->pPreUpdate;
#endif
  ...
  if( p->op==SQLITE_INSERT ){
    ...
    */
    ...
    if( !p->aNew ){
      ...
      p->aNew = static_cast<Mem*>(sqlite3DbMallocZero(db, sizeof(Mem)*p->pCsr->nField));
      if( !p->aNew ){
        rc = SQLITE_NOMEM;
        goto preupdate_new_out;
      }
...
void sqlite3_stmt_scanstatus_reset(sqlite3_stmt *pStmt){
  Vdbe * const p = (Vdbe*)pStmt;
  for(int ii=0; p!=0 && ii<p->nOp; ii++){
    Op * const pOp = &p->aOp[ii];
    pOp->nExec = 0;
    pOp->nCycle = 0;
  }
}
