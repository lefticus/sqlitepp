...
Select *sqlite3SelectNew(
  ...
  pNew->nSelectRow = 0;
  if( pSrc==0 ) pSrc = static_cast<SrcList*>(sqlite3DbMallocZero(pParse->db, SZ_SRCLIST_1));
  pNew->pSrc = pSrc;
  pNew->pWhere = pWhere;
...
int sqlite3ColumnIndex(Table *pTab, const char *zCol){
  int i;
  const u8 h = sqlite3StrIHash(zCol);
  const Column *const aCol = pTab->aCol;
  const int nCol = pTab->nCol;
...
void sqlite3SrcItemColumnUsed(SrcItem *pItem, int iCol){
  ...
  if( pItem->fg.isNestedFrom ){
    ...
    ExprList *const pResults = pItem->u4.pSubq->pSelect->pEList;
    assert( pResults!=0 );
...
static int sqlite3ProcessJoin(Parse *pParse, Select *p){
  SrcList *const pSrc = p->pSrc;  /* All tables in the FROM clause */
  int i, j;                       /* Loop counters */
  SrcItem *pLeft = &pSrc->a[0];  /* Left table being joined */
  SrcItem *pRight = &pLeft[1];   /* Right table being joined */
...
**
** Return the in which the result is stored.
*/
...
static void pushOntoSorter(
  ...
  int regData,           /* First holding data to be sorted */
  int regOrigData,       /* First holding data before packing */
...
**
**     The returned value in this case is the number of the first
**     in an array of registers used to store the previous result row so that
**     it can be compared to the next. The caller must ensure that this
**     is initialized to NULL.  (The fixDistinctOpenEph() routine
  ...
  if( pParse->nErr==0
   && (eTnctType==WHERE_DISTINCT_UNIQUE || eTnctType==WHERE_DISTINCT_ORDERED)
    ...
    if( eTnctType==WHERE_DISTINCT_ORDERED ){
      ...
      ** bit on the first of the previous value.  This will cause the
...
KeyInfo *sqlite3KeyInfoAlloc(sqlite3 *db, int N, int X){
  const int nExtra = (N+X)*(sizeof(CollSeq*)+1);
  assert( X>=0 );
  if( NEVER(N+X>0xffff) ) return static_cast<KeyInfo*>(sqlite3OomFault(db));
  KeyInfo *p = static_cast<KeyInfo*>(sqlite3DbMallocRawNN(db, SZ_KEYINFO(0) + nExtra));
  if( p ){
    p->aSortFlags = reinterpret_cast<u8*>(&p->aColl[N+X]);
    p->nKeyField = (u16)N;
    p->nAllField = (u16)(N+X);
    p->enc = ENC(db);
    p->db = db;
    ...
    return static_cast<KeyInfo*>(sqlite3OomFault(db));
  }
  return p;
}
...
int sqlite3ColumnsFromExprList(
  ...
  if( pEList ){
    nCol = pEList->nExpr;
    aCol = static_cast<Column*>(sqlite3DbMallocZero(db, sizeof(aCol[0])*nCol));
    testcase( aCol==0 );
  ...
  *paCol = aCol;
  ...
  for(i=0, pCol=aCol; i<nCol && !pParse->nErr; i++, pCol++){
    ...
    if( (zName = pX->zEName)!=0 && pX->fg.eEName==ENAME_NAME ){
      ...
      if( pColExpr->op==TK_COLUMN
       ...
       && ALWAYS( pColExpr->y.pTab!=0 )
        ...
        if( iCol<0 ) iCol = pTab->iPKey;
        zName = iCol>=0 ? pTab->aCol[iCol].zCnName : const_cast<char*>("rowid");
    ...
    */
    cnt = 0;
    while( zName && (pCollide = static_cast<struct ExprList_item*>(sqlite3HashFind(&ht, zName)))!=0 ){
...
void sqlite3SubqueryColumnTypes(
  ...
  for(i=0, pCol=pTab->aCol; i<pTab->nCol; i++, pCol++){
    ...
    if( zType ){
      ...
      pCol->zCnName = static_cast<char*>(sqlite3DbReallocOrFree(db, pCol->zCnName, n+k+2));
...
Table *sqlite3ResultSetOfSelect(Parse *pParse, Select *pSelect, char aff){
  ...
  while( pSelect->pPrior ) pSelect = pSelect->pPrior;
  Table *pTab = static_cast<Table*>(sqlite3DbMallocZero(db, sizeof(Table) ));
  if( pTab==0 ){
    return 0;
  }
  pTab->nTabRef = 1;
...
** are the integer memory numbers for counters used to compute
...
**
** The iOffset (if it exists) is initialized to the value
** of the OFFSET.  The iLimit is initialized to LIMIT.  Register
  ...
  */
  if( pLimit ){
    ...
    if( pLimit->pRight ){
      ...
      pParse->nMem++;   /* Allocate an extra for limit+offset */
...
static KeyInfo *multiSelectByMergeKeyInfo(Parse *pParse, Select *p, int nExtra){
  ExprList *const pOrderBy = p->pOrderBy;
  const int nOrderBy = (pOrderBy!=0) ? pOrderBy->nExpr : 0;
  sqlite3 *const db = pParse->db;
    ...
    */
    ...
    if( pKeyInfo ){
      ...
      sqlite3VdbeAddOp4(v, OP_OpenEphemeral, iDistinct, nCol, 0,
                           reinterpret_cast<const char*>(pKeyInfo), P4_KEYINFO);
    }else{
...
** starting at pIn->iSdst.  pDest is where the output should
** be sent.
**
** regReturn is the number of the holding the subroutine
** return address.
**
** If regPrev>0 then it is the first in a vector that
...
static int generateOutputSubroutine(
  ...
  int regReturn,          /* The return address */
  ...
  */
  if( regPrev ){
    ...
    addr2 = sqlite3VdbeAddOp4(v, OP_Compare, pIn->iSdst, regPrev+1, pIn->nSdst,
                              reinterpret_cast<char*>(sqlite3KeyInfoRef(pKeyInfo)), P4_KEYINFO);
...
** within the output subroutine.  The regPrev set holds the previously
...
static int multiSelectByMerge(
  ...
  int regAddrA;         /* Address for select-A coroutine */
  int regAddrB;         /* Address for select-B coroutine */
  int addrSelectA;      /* Address of the select-A coroutine */
  int addrSelectB;      /* Address of the select-B coroutine */
  int regOutA;          /* Address for the output-A subroutine */
  int regOutB;          /* Address for the output-B subroutine */
  ...
  int regLimitA;        /* Limit for select-A */
  int regLimitB;        /* Limit for select-A */
  ...
  */
  aPermute = static_cast<u32*>(sqlite3DbMallocRawNN(db, sizeof(u32)*(nOrderBy + 1)));
  if( aPermute ){
    struct ExprList_item *pItem;
    int bKeep = 0;
    aPermute[0] = nOrderBy;
...
** flag is set, then the value in the is set to NULL and the original
...
static int recomputeColumnsUsedExpr(Walker *pWalker, Expr *pExpr){
  ...
  SrcItem *const pItem = pWalker->u.pSrcItem;
...
static void renumberCursorDoMapping(Walker *pWalker, int *piCursor){
  int *const aCsrMap = pWalker->u.aiCol;
  const int iCsr = *piCursor;
...
static int compoundHasDifferentAffinities(Select *p){
  ...
  assert( p->pEList!=0 );
  assert( p->pPrior!=0 );
  ExprList *const pList = p->pEList;
  for(int ii=0; ii<pList->nExpr; ii++){
    assert( pList->a[ii].pExpr!=0 );
    const char aff = sqlite3ExprAffinity(pList->a[ii].pExpr);
    for(Select *pSub1=p->pPrior; pSub1; pSub1=pSub1->pPrior){
  ...
  */
  if( pSub->pPrior ){
    ...
    if( pSrc->nSrc>1 ){
      ...
      aCsrMap = static_cast<int*>(sqlite3DbMallocZero(db, ((i64)pParse->nTab+1)*sizeof(int)));
...
static void constInsert(
  ...
  ** Make sure the same pColumn is not inserted more than once */
  ...
  pConst->nConst++;
  pConst->apExpr = static_cast<Expr**>(sqlite3DbReallocOrFree(pConst->pParse->db, pConst->apExpr,
...
static int propagateConstantExprRewrite(Walker *pWalker, Expr *pExpr){
  WhereConst *const pConst = pWalker->u.pConst;
...
static int disableUnusedSubqueryResultColumns(SrcItem *pItem){
  ...
  Table *const pTab = pItem->pSTab;
  assert( pItem->fg.isSubquery );
  Select *const pSub = pItem->u4.pSubq->pSelect;
  ...
  const int nCol = pTab->nCol;
  for(j=0; j<nCol; j++){
    const Bitmask m = j<BMS-1 ? MASKBIT(j) : TOPBIT;
...
static u8 minMaxQuery(sqlite3 *db, Expr *pFunc, ExprList **ppMinMax){
  ...
  ExprList *const pEList = pFunc->x.pList;
  if( pEList==0
   || pEList->nExpr!=1
  ...
  const char *const zFunc = pFunc->u.zToken;
...
static Table *isSimpleCount(Select *p, AggInfo *pAggInfo){
  ...
  Table *const pTab = p->pSrc->a[0].pSTab;
  assert( pTab!=0 );
  assert( !IsView(pTab) );
  if( !IsOrdinaryTable(pTab) ) return 0;
  Expr *const pExpr = p->pEList->a[0].pExpr;
  assert( pExpr!=0 );
...
int sqlite3IndexedByLookup(Parse *pParse, SrcItem *pFrom){
  Table *const pTab = pFrom->pSTab;
  char *const zIndexedBy = pFrom->u1.zIndexedBy;
  assert( pTab!=0 );
...
static int convertCompoundSelectToSubquery(Walker *pWalker, Select *p){
  ...
  struct ExprList_item *const a = p->pOrderBy->a;
#ifndef SQLITE_OMIT_WINDOWFUNC
  ...
  ** by this function is not required in this case. */
  ...
  sqlite3 *const db = pParse->db;
  Select *pNew = static_cast<Select*>(sqlite3DbMallocZero(db, sizeof(*pNew) ));
  if( pNew==0 ) return WRC_Abort;
  Token dummy;
  memset(&dummy, 0, sizeof(dummy));
  SrcList *const pNewSrc = sqlite3SrcListAppendFromTerm(pParse,0,0,0,&dummy,pNew,0);
...
With *sqlite3WithPush(Parse *pParse, With *pWith, u8 bFree){
  if( pWith ){
    if( bFree ){
      pWith = static_cast<With*>(sqlite3ParserAddCleanup(pParse, sqlite3WithDeleteGeneric,
                      pWith));
      if( pWith==0 ) return 0;
    }
    if( pParse->nErr==0 ){
...
static int resolveFromTermToCte(
  ...
  if( pCte ){
    ...
    ** In this case, proceed.  */
    ...
    pCteUse = pCte->pUse;
    if( pCteUse==0 ){
      pCte->pUse = pCteUse = static_cast<CteUse*>(sqlite3DbMallocZero(db, sizeof(pCteUse[0])));
      if( pCteUse==0
...
int sqlite3ExpandSubquery(Parse *pParse, SrcItem *pFrom){
  ...
  assert( pSel );
  Table *pTab;
  pFrom->pSTab = pTab = static_cast<Table*>(sqlite3DbMallocZero(pParse->db, sizeof(Table)));
...
static int selectExpander(Walker *pWalker, Select *p){
  ...
  if( pParse->pWith && (p->selFlags & SF_View) ){
    if( p->pWith==0 ){
      p->pWith = static_cast<With*>(sqlite3DbMallocZero(db, SZ_WITH(1) ));
      if( p->pWith==0 ){
        return WRC_Abort;
      }
    }
...
static int aggregateIdxEprRefToColCallback(Walker *pWalker, Expr *pExpr){
  ...
  AggInfo *const pAggInfo = pExpr->pAggInfo;
  if( NEVER(pExpr->iAgg>=pAggInfo->nColumn) ) return WRC_Continue;
  assert( pExpr->iAgg>=0 );
  struct AggInfo_col *const pCol = &pAggInfo->aCol[pExpr->iAgg];
...
/*
** Allocate a block of registers so that there is one for each
** pAggInfo->aCol[] and pAggInfo->aFunc[] entry in pAggInfo.  The first
** in this block is stored in pAggInfo->iFirstReg.
**
...
static void resetAccumulator(Parse *pParse, AggInfo *pAggInfo){
  ...
  const int nReg = pAggInfo->nFunc + pAggInfo->nColumn;
  assert( pAggInfo->iFirstReg>0 );
  assert( pParse->db->pParse==pParse );
...
** registers if regAcc contains 0. The caller will take care
  ...
  for(i=0, pF=pAggInfo->aFunc; i<pAggInfo->nFunc; i++, pF++){
    ...
    if( ExprHasProperty(pF->pFExpr, EP_WinFunc) ){
      ...
      if( pAggInfo->nAccumulator
        ...
        ** "magnet" regHit so that the accumulator registers
        ** are populated if the FILTER clause jumps over the the
        ** invocation of min() or max() altogether. Or, if this is not
        ** the first row (regAcc contains 1), set the magnet so that
    ...
    if( pF->iOBTab>=0 ){
      ...
      ** have been passed to AggStep onto the sorting table. */
      ...
      regAggSz++;  /* One extra to hold result of MakeRecord */
...
static int havingToWhereExprCb(Walker *pWalker, Expr *pExpr){
  if( pExpr->op!=TK_AND ){
    Select *const pS = pWalker->u.pSelect;
...
static int countOfViewOptimization(Parse *pParse, Select *p){
  ...
  sqlite3 *const db = pParse->db;
  Expr *const pCount = pExpr;
  pExpr = 0;
  pSub = sqlite3SubqueryDetach(db, pFrom);
  sqlite3SrcListDelete(db, p->pSrc);
  p->pSrc = static_cast<SrcList*>(sqlite3DbMallocZero(pParse->db, SZ_SRCLIST_1));
  while( pSub ){
    Expr *pTerm;
    pPrior = pSub->pPrior;
    pSub->pPrior = 0;
...
** the where.cpp file to exit the corresponding loop after the first successful
** match (if any).
*/
    ...
    if( pWhere->op==TK_AND ){
      ...
      if( pSub->pSrc->nSrc==1
        ...
        ** a unique cursor number.  */
        sqlite3 *db = pParse->db;
        int *aCsrMap = static_cast<int*>(sqlite3DbMallocZero(db, (pParse->nTab+2)*sizeof(int)));
        if( aCsrMap==0 ) return;
...
static int selectCheckOnClausesSelect(Walker *pWalker, Select *pSelect){
  CheckOnCtx *const pCtx = pWalker->u.pCheckOnCtx;
...
static int sqlite3CopySortOrder(ExprList *p1, ExprList *p2){
  ...
  if( p2 && p1->nExpr==p2->nExpr ){
    ...
    for(ii=0; ii<p1->nExpr; ii++){
      const u8 sortFlags = p2->a[ii].fg.sortFlags & KEYINFO_ORDER_DESC;
  ...
  */
  if( p->selFlags & SF_Distinct ){
    ...
    sDistinct.addrTnct = sqlite3VdbeAddOp4(v, OP_OpenEphemeral,
                       ...
                       reinterpret_cast<char*>(sqlite3KeyInfoFromExprList(pParse, p->pEList,0,0)),
                       P4_KEYINFO);
  ...
  if( !isAgg && pGroupBy==0 ){
    ...
    ** clause or both.  tag-select-0800 */
    ...
    int sortOut = 0;    /* Output from the sorter */
    ...
    ** SELECT statement.
    */
    pAggInfo = static_cast<AggInfo*>(sqlite3DbMallocZero(db, sizeof(*pAggInfo) ));
    if( pAggInfo ){
    ...
    */
    if( pGroupBy ){
      ...
      int regOutputRow;   /* Return address for output subroutine */
      int addrSetAbort;   /* Set the abort flag and return */
      int addrTopOfLoop;  /* Top of the input loop */
      int addrSortingIdx; /* The OP_OpenEphemeral for the sorting index */
      int addrReset;      /* Subroutine for resetting the accumulator */
      int regReset;       /* Return address for reset subroutine */
      ...
      */
      ...
      if( (pTab = isSimpleCount(p, pAggInfo))!=0 ){
        ...
        ** without FILTER clauses, allocate regAcc. Register regAcc
