...
static int memdbOpen(
  ...
  if( szName>1 && (zName[0]=='/' || zName[0]=='\\') ){
    ...
    if( p==0 ){
      MemStore **apNew;
      p = static_cast<MemStore*>(sqlite3Malloc( sizeof(*p) + (i64)szName + 3 ));
      if( p==0 ){
        sqlite3_mutex_leave(pVfsMutex);
        return SQLITE_NOMEM;
      }
      apNew = static_cast<MemStore**>(sqlite3Realloc(memdb_g.apMemStore,
    ...
    p = static_cast<MemStore*>(sqlite3Malloc( sizeof(*p) ));
    if( p==0 ){
      return SQLITE_NOMEM;
    }
    memset(p, 0, sizeof(*p));
...
unsigned char *sqlite3_serialize(
  ...
  if( p ){
    ...
    if( mFlags & SQLITE_SERIALIZE_NOCOPY ){
      ...
      pOut = static_cast<unsigned char*>(sqlite3_malloc64( pStore->sz ));
