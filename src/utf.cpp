...
*/
constexpr inline unsigned char sqlite3Utf8Trans1[] = {
...
SQLITE_NOINLINE int sqlite3VdbeMemTranslate(Mem *pMem, u8 desiredEnc){
  ...
  */
  zIn = (u8*)pMem->z;
  zTerm = &zIn[pMem->n];
  zOut = static_cast<unsigned char *>(sqlite3DbMallocRaw(pMem->db, len));
  if( !zOut ){
    return SQLITE_NOMEM_BKPT;
  }
  z = zOut;
...
char *sqlite3Utf16to8(sqlite3 *db, const void *z, int nByte, u8 enc){
  ...
  memset(&m, 0, sizeof(m));
  m.db = db;
  sqlite3VdbeMemSetStr(&m, static_cast<const char*>(z), nByte, enc, SQLITE_STATIC);
...
int sqlite3Utf16ByteLen(const void *zIn, int nByte, int nChar){
  int c;
  unsigned char const *z = static_cast<const unsigned char*>(zIn);
