...
static void DbUnlockNotify(void **apArg, int nArg){
  for(int i=0; i<nArg; i++){
    constexpr int flags = (TCL_EVAL_GLOBAL|TCL_EVAL_DIRECT);
...
static void DbPreUpdateHandler(
  ...
  Tcl_Obj *pCmd;
  static constexpr std::array<const char *, 3> azStr = {"DELETE", "INSERT", "UPDATE"};
...
static int SQLITE_TCLAPI DbTransPostCmd(
  ...
  static constexpr std::array<const char *const, 4> azEnd = {
...
static int dbQrf(SqliteDb *pDb, int objc, Tcl_Obj *const*objv){
  ...
  static constexpr std::array<const char *, 17> azAlign = {
    "auto",           "bottom",          "c",
    "center",         "e",               "left",
    "middle",         "n",               "ne",
    "nw",             "right",           "s",
  ...
  static constexpr std::array<unsigned char, 16> aAlignMap = {
  ...
  for(int i=2; i<objc; i++){
    ...
    constexpr std::array<const char *, 6> azBool = { "auto", "yes", "no", "on", "off", nullptr };
    constexpr std::array<unsigned char, 5> aBoolMap = { 0, 2, 1, 2, 1 };
    if( zArg[0]!='-' ){
      if( zSql ){
      ...
      static constexpr std::array<const char *, 20> azStyles = {
        "auto",             "box",              "column",
        "count",            "csv",              "eqp",
        "explain",          "html",             "insert",
        "jobject",          "json",             "line",
      ...
      static constexpr std::array<unsigned char, 19> aStyleMap = {
      ...
      static constexpr const char *azEsc[] = {
      ...
      ** thrid element of the array when processing --text */
      static constexpr const char *azText[] = {           "off",   "on",
        "auto",             "csv",              "html",
        "json",             "plain",            "relaxed",
        "sql",              "tcl",              0
      };
      ...
      static constexpr const char *azBlob[] = {
        "auto",             "hex",              "json",
        "tcl",              "text",             "sql",
        "size",             0
      };
      ...
      qrf.aAlign = (unsigned char*)sqlite3_malloc64( (n+1)*sizeof(qrf.aAlign[0]) );
      if( qrf.aAlign==0 ){
      ...
      qrf.aWidth = (short int*)sqlite3_malloc64( (n+1)*sizeof(qrf.aWidth[0]) );
      if( qrf.aWidth==0 ){
        Tcl_AppendResult(pDb->interp, "out of memory", (char*)0);
        rc = TCL_ERROR;
        goto format_failed;
...
static int SQLITE_TCLAPI DbObjCmd(
  ...
  int choice;
  int rc = TCL_OK;
  static constexpr const char *DB_strs[] = {
  ...
  */
  case DB_CONFIG: {
    static constexpr struct DbConfigChoices {
      const char *zName;
      int op;
    } aDbConfig[] = {
  ...
  */
  case DB_COPY: {
    ...
    zSql = (char*)malloc( nByte + 50 + nCol*2 );
    if( zSql==0 ) {
  ...
  */
  case DB_DESERIALIZE: {
    ...
    pData = (unsigned char*)sqlite3_malloc64( len );
    if( pData==0 && len>0 ){
...
deserialize_error:
  ...
  case DB_FUNCTION: {
    ...
    for(i=3; i<(objc-1); i++){
      ...
      if( n>1 && strncmp(z, "-returntype", n)==0 ){
        constexpr const char *azType[] = {"integer", "real", "text", "blob", "any", 0};
  ...
  */
  case DB_TRACE_V2: {
    if( objc>4 ){
      ...
      Tcl_WideInt wMask = 0;
      if( objc==4 ){
        static constexpr const char *TTYPE_strs[] = {
  ...
  */
  case DB_PREUPDATE: {
    ...
    rc = TCL_ERROR;
#else
    static constexpr const char *azSub[] = {"count", "depth", "hook", "new", "old", 0};
  ...
  */
  ...
  if( DbUseNre() ){
    Tcl_NRCreateCommand(interp, zArg, DbObjCmdAdaptor, DbObjCmd,
                        (ClientData)p, DbDeleteCmd);
  }else{
    Tcl_CreateObjCommand(interp, zArg, DbObjCmd, (ClientData)p, DbDeleteCmd);
  }
  p->nRef = 1;
  return TCL_OK;
}
...
*/
int Sqlite3_SafeInit(Tcl_Interp *interp){ return TCL_ERROR; }
int Sqlite3_SafeUnload(Tcl_Interp *interp, int flags){return TCL_ERROR;}
...
static const char *tclsh_main_loop(void){
  static constexpr char zMainloop[] =
    "if {[llength $argv]>=1} {\n"
#ifdef WIN32
      "set new [list]\n"
      "foreach arg $argv {\n"
