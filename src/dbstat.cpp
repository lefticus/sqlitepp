      ...
      if( p->flags==0x05 ){
        ...
        if( nPayload>(u32)nLocal ){
          ...
          for(int j=1; j<nOvfl; j++){
            ...
            pCell->aOvfl[j] = sqlite3Get4byte(static_cast<const u8*>(sqlite3PagerGetData(pPg)));
...
static int statGetPage(
  ...
  if( rc==SQLITE_OK ){
    const u8 *a = static_cast<const u8*>(sqlite3PagerGetData(pDbPage));
...
/*
** Invoke this routine to the "dbstat" virtual table module
*/
