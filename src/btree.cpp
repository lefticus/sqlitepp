...
** See the header comment on "btreeInt.hpp" for additional information.
...
static int querySharedCacheTableLock(Btree *p, Pgno iTab, u8 eLock){
  ...
  */
  ...
  for(BtLock *pIter=pBt->pLock; pIter; pIter=pIter->pNext){
...
static int setSharedCacheTableLock(Btree *p, Pgno iTable, u8 eLock){
  ...
  */
  if( !pLock ){
    pLock = static_cast<BtLock*>(sqlite3MallocZero(sizeof(BtLock)));
    if( !pLock ){
...
static void downgradeAllSharedCacheTableLocks(Btree *p){
  ...
  if( pBt->pWriter==p ){
    pBt->pWriter = 0;
    pBt->btsFlags &= ~(BTS_EXCLUSIVE|BTS_PENDING);
    for(BtLock *pLock=pBt->pLock; pLock; pLock=pLock->pNext){
...
static void invalidateAllOverflowCache(BtShared *pBt){
  ...
  for(BtCursor *p=pBt->pCursor; p; p=p->pNext){
...
static int btreeGetHasContent(BtShared *pBt, Pgno pgno){
  Bitvec *const p = pBt->pHasContent;
...
static int saveCursorPosition(BtCursor *pCur){
  ...
  const int rc = saveCursorKey(pCur);
  if( rc==SQLITE_OK ){
...
static int getAndInitPage(
  ...
  pPage = static_cast<MemPage*>(sqlite3PagerGetExtra(pDbPage));
  if( pPage->isInit==0 ){
...
int sqlite3BtreeOpen(
  ...
  */
  ...
  p = static_cast<Btree*>(sqlite3MallocZero(sizeof(Btree)));
  if( !p ){
    return SQLITE_NOMEM_BKPT;
  }
  ...
  */
  if( isTempDb==0 && (isMemdb==0 || (vfsFlags&SQLITE_OPEN_URI)!=0) ){
    if( vfsFlags & SQLITE_OPEN_SHAREDCACHE ){
      ...
      char *zFullPathname = static_cast<char*>(sqlite3Malloc(MAX(nFullPathname,nFilename)));
...
static SQLITE_NOINLINE int allocateTempSpace(BtShared *pBt){
  ...
  ** first write cursor for the BtShared object */
  ...
  pBt->pTmpSpace = static_cast<u8*>(sqlite3PageMalloc( pBt->pageSize ));
  if( pBt->pTmpSpace==0 ){
      ...
      if( offset>=ovflSize ){
        ...
        */
...

        {
          ...
          if( rc==SQLITE_OK ){
            aPayload = static_cast<unsigned char*>(sqlite3PagerGetData(pDbPage));
...
static SQLITE_NOINLINE int accessPayloadChecked(
  ...
  return rc ? rc : accessPayload(pCur, offset, amt, static_cast<unsigned char*>(pBuf), 0);
}
int sqlite3BtreePayloadChecked(BtCursor *pCur, u32 offset, u32 amt, void *pBuf){
  if( pCur->eState==CURSOR_VALID ){
    assert( cursorOwnsBtShared(pCur) );
    return accessPayload(pCur, offset, amt, static_cast<unsigned char*>(pBuf), 0);
  }else{
...
static int fillInCell(
  ...
  ** buffer space that is separate from the pPage buffer area */
  ...
  if( pPage->intKey ){
    ...
    pSrc = static_cast<const u8*>(pX->pData);
    nSrc = pX->nData;
    ...
    pSrc = static_cast<const u8*>(pX->pKey);
    nHeader += putVarint32(&pCell[nHeader], nPayload);
  }
...
static int rebuildPage(
  ...
  u8 *pTmp = static_cast<u8*>(sqlite3PagerTempSpace(pPg->pBt->pPager));
  u8 *pData;
      ...
      ** of its own ancestor pages. Corruption. */
      ...
      if( rc==SQLITE_OK ){
...
#endif
        {
          ...
          */
          u8 *pSpace = static_cast<u8*>(sqlite3PageMalloc(pCur->pBt->pageSize));
...
static void checkList(
  ...
  while( iPage!=0 && pCheck->mxErr ){
    ...
    pOvflData = static_cast<unsigned char*>(sqlite3PagerGetData(pOvflPage));
    if( isFreeList ){
...
int sqlite3BtreeIntegrityCheck(
  ...
  sCheck.aPgRef = static_cast<u8*>(sqlite3MallocZero((sCheck.nCkPage / 8)+ 1));
  if( !sCheck.aPgRef ){
    checkOom(&sCheck);
    goto integrity_ck_cleanup;
  }
