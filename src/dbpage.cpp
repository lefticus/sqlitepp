...
#include "sqliteInt.hpp"   /* Requires access to internal data structures */
...
static int dbpageUpdate(
  ...
  if( rc==SQLITE_OK ){
    const void *pData = static_cast<const void *>(sqlite3_value_blob(argv[3]));
    if( (rc = sqlite3PagerWrite(pDbPage))==SQLITE_OK && pData ){
      unsigned char *aPage = static_cast<unsigned char*>(sqlite3PagerGetData(pDbPage));
...
/*
** Invoke this routine to the "dbpage" virtual table module
*/
