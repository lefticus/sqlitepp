...
static void instrFunc(
  ...
  if( nNeedle>0 ){
    if( typeHaystack==SQLITE_BLOB && typeNeedle==SQLITE_BLOB ){
      zHaystack = static_cast<const unsigned char *>(sqlite3_value_blob(argv[0]));
      zNeedle = static_cast<const unsigned char *>(sqlite3_value_blob(argv[1]));
      isText = 0;
...
static void substrFunc(
  ...
  if( p0type==SQLITE_BLOB ){
    ...
    z = static_cast<const unsigned char *>(sqlite3_value_blob(argv[0]));
    if( z==0 ) return;
...
static void *contextMalloc(sqlite3_context *context, i64 nByte){
  ...
  if( nByte>db->aLimit[SQLITE_LIMIT_LENGTH] ){
    ...
    z = static_cast<char *>(sqlite3Malloc(nByte));
    if( !z ){
      sqlite3_result_error_nomem(context);
    }
  }
...
static void randomBlob(
  ...
  p = static_cast<unsigned char*>(contextMalloc(context, n));
  if( p ){
...
static void likeFunc(
  ...
  int nPat;
  sqlite3 *db = sqlite3_context_db_handle(context);
  struct compareInfo *pInfo = static_cast<struct compareInfo *>(sqlite3_user_data(context));
...
void sqlite3QuoteValue(StrAccum *pStr, sqlite3_value *pValue, int bEscape){
  ...
  ** require enhancements to the implementation. */
  ...
  switch( sqlite3_value_type(pValue) ){
    ...
    case SQLITE_BLOB: {
      char const *zBlob = static_cast<char const *>(sqlite3_value_blob(pValue));
...
static void unistrFunc(
  ...
  zOut = static_cast<char *>(sqlite3_malloc64(nIn+1));
  if( zOut==0 ){
    sqlite3_result_error_nomem(context);
    return;
  }
...
static void charFunc(
  ...
  int i;
  zOut = z = static_cast<unsigned char *>(sqlite3_malloc64( argc*4+1 ));
  if( z==0 ){
    sqlite3_result_error_nomem(context);
    return;
  }
...
static void hexFunc(
  ...
  assert( argc==1 );
  UNUSED_PARAMETER(argc);
  pBlob = static_cast<const unsigned char *>(sqlite3_value_blob(argv[0]));
  n = sqlite3_value_bytes(argv[0]);
  assert( pBlob==sqlite3_value_blob(argv[0]) );  /* No encoding change */
  z = zHex = static_cast<char*>(contextMalloc(context, ((i64)n)*2 + 1));
  if( zHex ){
    for(i=0; i<n; i++, pBlob++){
...
static void unhexFunc(
  ...
  p = pBlob = static_cast<u8*>(contextMalloc(pCtx, (nHex/2)+1));
  if( pBlob ){
...
static void replaceFunc(
  ...
  zOut = static_cast<unsigned char*>(contextMalloc(context, nOut));
  if( zOut==0 ){
    return;
  }
      ...
      if( nRep>nPattern ){
        ...
        if( (cntExpand&(cntExpand-1))==0 ){
          ...
          ** whose index is a power of two: 1, 2, 4, 8, 16, 32, ... */
          u8 *zOld;
          zOld = zOut;
          zOut = static_cast<unsigned char *>(sqlite3Realloc(zOut, (int)nOut + (nOut - nStr - 1)));
          if( zOut==0 ){
            sqlite3_result_error_nomem(context);
            sqlite3_free(zOld);
            return;
...
static void trimFunc(
  ...
  if( argc==1 ){
    ...
    if( nChar>0 ){
      azChar = static_cast<unsigned char**>(contextMalloc(context,
...
static void concatFuncCore(
  ...
  n += (argc-1)*(i64)nSep;
  z = static_cast<char *>(sqlite3_malloc64(n+1));
  if( z==0 ){
    sqlite3_result_error_nomem(context);
    return;
  }
...
static void minmaxStep(
  ...
  Mem *pArg  = static_cast<Mem*>(argv[0]);
  Mem *pBest;
  UNUSED_PARAMETER(NotUsed);

  pBest = static_cast<Mem*>(sqlite3_aggregate_context(context, sizeof(*pBest)));
  if( !pBest ) return;
...
static void groupConcatStep(
  ...
  assert( argc==1 || argc==2 );
  if( sqlite3_value_type(argv[0])==SQLITE_NULL ) return;
  pGCC = static_cast<GroupConcatCtx*>(sqlite3_aggregate_context(context, sizeof(*pGCC)));
  if( pGCC ){
    sqlite3 *db = sqlite3_context_db_handle(context);
    ...
    if( argc==1 ){
      ...
      zSep = reinterpret_cast<const char*>(sqlite3_value_text(argv[1]));
      ...
      if( nSep != pGCC->nFirstSepLength || pGCC->pnSepLengths != 0 ){
        ...
        if( pnsl == 0 ){
          ...
          pnsl = static_cast<int*>(sqlite3_malloc64((pGCC->nAccum+1) * sizeof(int)));
          if( pnsl!=0 ){
            int i = 0, nA = pGCC->nAccum-1;
            while( i<nA ) pnsl[i++] = pGCC->nFirstSepLength;
          }
        }else{
          pnsl = static_cast<int*>(sqlite3_realloc64(pnsl, pGCC->nAccum * sizeof(int)));
        }
        if( pnsl!=0 ){
    ...
    pGCC->nAccum += 1;
#endif
    zVal = reinterpret_cast<const char*>(sqlite3_value_text(argv[0]));
...
static void groupConcatFinalize(sqlite3_context *context){
  GroupConcatCtx *pGCC
    = static_cast<GroupConcatCtx*>(sqlite3_aggregate_context(context, 0));
  if( pGCC ){
...
/*
** Re-the built-in LIKE functions.  The caseSensitive
...
static void ceilingFunc(
  ...
  switch( sqlite3_value_numeric_type(argv[0]) ){
    ...
    case SQLITE_FLOAT: {
       double (*x)(double) = reinterpret_cast<double(*)(double)>(sqlite3_user_data(context));
...
static void math1Func(
  ...
  x = reinterpret_cast<double(*)(double)>(sqlite3_user_data(context));
  ans = x(v0);
  sqlite3_result_double(context, ans);
}
...
static void math2Func(
  ...
  x = reinterpret_cast<double(*)(double,double)>(sqlite3_user_data(context));
  ans = x(v0, v1);
  sqlite3_result_double(context, ans);
}
...
static void percentStep(sqlite3_context *pCtx, int argc, sqlite3_value **argv){
  ...
  ** Requirement 4 */
  ...
  if( p->nUsed>=p->nAlloc ){
    ...
    double *a = static_cast<double *>(sqlite3_realloc64(p->a, sizeof(double)*n));
    if( a==0 ){
      sqlite3_free(p->a);
      memset(p, 0, sizeof(*p));
...
/***** Regular functions *****/
    ...
    MFUNCTION(trunc,             1, (double(*)(double))trunc,     ceilingFunc ),
#endif
    ...
    MFUNCTION(exp,               1, (double(*)(double))exp,       math1Func   ),
    MFUNCTION(pow,               2, (double(*)(double,double))pow,       math2Func   ),
    MFUNCTION(power,             2, (double(*)(double,double))pow,       math2Func   ),
    MFUNCTION(mod,               2, (double(*)(double,double))fmod,      math2Func   ),
    MFUNCTION(acos,              1, (double(*)(double))acos,      math1Func   ),
    MFUNCTION(asin,              1, (double(*)(double))asin,      math1Func   ),
    MFUNCTION(atan,              1, (double(*)(double))atan,      math1Func   ),
    MFUNCTION(atan2,             2, (double(*)(double,double))atan2,     math2Func   ),
    MFUNCTION(cos,               1, (double(*)(double))cos,       math1Func   ),
    MFUNCTION(sin,               1, (double(*)(double))sin,       math1Func   ),
    MFUNCTION(tan,               1, (double(*)(double))tan,       math1Func   ),
    MFUNCTION(cosh,              1, (double(*)(double))cosh,      math1Func   ),
    MFUNCTION(sinh,              1, (double(*)(double))sinh,      math1Func   ),
    MFUNCTION(tanh,              1, (double(*)(double))tanh,      math1Func   ),
#if SQLITE_HAVE_C99_MATH_FUNCS
    MFUNCTION(acosh,             1, (double(*)(double))acosh,     math1Func   ),
    MFUNCTION(asinh,             1, (double(*)(double))asinh,     math1Func   ),
    MFUNCTION(atanh,             1, (double(*)(double))atanh,     math1Func   ),
#endif
    MFUNCTION(sqrt,              1, (double(*)(double))sqrt,      math1Func   ),
