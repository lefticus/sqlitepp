...
static int setupLookaside(
  ...
  ** more than 0x7fff0000 total bytes for lookaside. */
  ...
  if( pStart ){
    ...
    p = static_cast<LookasideSlot*>(pStart);
    for(i=0; i<nBig; i++){
      p->pNext = db->lookaside.pInit;
      db->lookaside.pInit = p;
      p = reinterpret_cast<LookasideSlot*>(&(reinterpret_cast<u8*>(p))[sz]);
    }
#ifndef SQLITE_OMIT_TWOSIZE_LOOKASIDE
    ...
    for(i=0; i<nSm; i++){
      ...
      p = reinterpret_cast<LookasideSlot*>(&(reinterpret_cast<u8*>(p))[LOOKASIDE_SMALL]);
    }
#endif /* SQLITE_OMIT_TWOSIZE_LOOKASIDE */
...
static void disconnectAllVtab(sqlite3 *db){
  ...
  for(int i=0; i<db->nDb; i++){
    ...
    if( pSchema ){
      for(HashElem *p=sqliteHashFirst(&pSchema->tblHash); p; p=sqliteHashNext(p)){
        Table *pTab = (Table *)sqliteHashData(p);
        if( IsVirtual(pTab) ) sqlite3VtabDisconnect(db, pTab);
      }
    }
  }
  for(HashElem *p=sqliteHashFirst(&db->aModule); p; p=sqliteHashNext(p)){
...
void sqlite3LeaveMutexAndCloseZombie(sqlite3 *db){
  ...
  */
  ...
  for(HashElem *i=sqliteHashFirst(&db->aFunc); i; i=sqliteHashNext(i)){
    FuncDef *pNext, *p;
    p = static_cast<FuncDef*>(sqliteHashData(i));
    do{
  ...
  for(HashElem *i=sqliteHashFirst(&db->aCollSeq); i; i=sqliteHashNext(i)){
...
#ifndef SQLITE_OMIT_VIRTUALTABLE
  for(HashElem *i=sqliteHashFirst(&db->aModule); i; i=sqliteHashNext(i)){
...
void sqlite3RollbackAll(sqlite3 *db, int tripCode){
  ...
  ** corruption reports in some cases.  */
  sqlite3BtreeEnterAll(db);
  const int schemaChange = (db->mDbFlags & DBFLAG_SchemaChange)!=0 && db->init.busy==0;
...
const char *sqlite3ErrName(int rc){
  const char *zName = 0;
  const int origRc = rc;
  for(int i=0; i<2 && zName==0; i++, rc &= 0xff){
    switch( rc ){
...
int sqlite3InvokeBusyHandler(BusyHandler *p){
  ...
  const int rc = p->xBusyHandler(p->pBusyArg, p->nBusy);
  if( rc==0 ){
    p->nBusy = -1;
  }else{
    p->nBusy++;
...
#ifdef SQLITE_ENABLE_SETLK_TIMEOUT
  const int bBOC = ((flags & SQLITE_SETLK_BLOCK_ON_CONNECT) ? 1 : 0);
  sqlite3_mutex_enter(db->mutex);
  db->setlkTimeout = ms;
  db->setlkFlags = flags;
  sqlite3BtreeEnterAll(db);
...
static int createFunctionApi(
  ...
  sqlite3_mutex_enter(db->mutex);
  if( xDestroy ){
    pArg = static_cast<FuncDestructor*>(sqlite3Malloc(sizeof(FuncDestructor)));
    if( !pArg ){
      sqlite3OomFault(db);
      xDestroy(p);
      goto out;
...
static void sqlite3InvalidFunction(
  ...
  const char *zName = static_cast<const char*>(sqlite3_user_data(context));
  char *zErr;
...
int sqlite3Checkpoint(sqlite3 *db, int iDb, int eMode, int *pnLog, int *pnCkpt){
  ...
  for(int i=0; i<db->nDb && rc==SQLITE_OK; i++){
...
const char *sqlite3_errmsg(sqlite3 *db){
  ...
  if( db->mallocFailed ){
    ...
    testcase( db->pErr==0 );
    z = db->errCode ? reinterpret_cast<const char*>(sqlite3_value_text(db->pErr)) : 0;
    assert( !db->mallocFailed );
    if( z==0 ){
...
const void *sqlite3_errmsg16(sqlite3 *db){
  ...
  const void *z;
  if( !db ){
    return static_cast<const void*>(outOfMem);
  }
  if( !sqlite3SafetyCheckSickOrOk(db) ){
    return static_cast<const void*>(misuse);
  }
  sqlite3_mutex_enter(db->mutex);
  if( db->mallocFailed ){
    z = static_cast<const void*>(outOfMem);
  }else{
  ...
  if( pColl && pColl->xCmp ){
    ...
    */
    if( (pColl->enc & ~SQLITE_UTF16_ALIGNED)==enc2 ){
      CollSeq *aColl = static_cast<CollSeq *>(sqlite3HashFind(&db->aCollSeq, zName));
      int j;
      for(j=0; j<3; j++){
        CollSeq *p = &aColl[j];
        if( p->enc==pColl->enc ){
...
int sqlite3_limit(sqlite3 *db, int limitId, int newLimit){
  ...
  */
  ...
  const int oldLimit = db->aLimit[limitId];
...
int sqlite3ParseUri(
  ...
  if( ((flags & SQLITE_OPEN_URI)                     /* IMP: R-48725-32206 */
    ...
    ** method that there may be extra parameters following the file-name.  */
    ...
    zFile = static_cast<char *>(sqlite3_malloc64(nByte));
    ...
    ** method. */
    ...
    zFile = static_cast<char *>(sqlite3_malloc64(nUri+8));
    if( !zFile ) return SQLITE_NOMEM_BKPT;
    memset(zFile, 0, 4);
    zFile += 4;
    if( nUri ){
...
static int openDatabase(
  ...
  */
  ...
  db = static_cast<sqlite3 *>(sqlite3MallocZero( sizeof(sqlite3) ));
  ...
  ** the temporary files used to do external sorts (see code in vdbesort.cpp)
...
int sqlite3_open16(
  ...
  *ppDb = 0;
  ...
  zFilename8 = static_cast<const char*>(sqlite3ValueText(pVal, SQLITE_UTF8));
  if( zFilename8 ){
...
#endif
  sqlite3_mutex_enter(db->mutex);
  for(DbClientData *p=db->pDbData; p; p=p->pNext){
...
int sqlite3_set_clientdata(
  ...
  if( p ){
    ...
    p = static_cast<DbClientData *>(sqlite3_malloc64( SZ_DBCLIENTDATA(n+1) ));
    if( p==0 ){
...
int sqlite3_sleep(int ms){
  ...
  */
  const int rc = (sqlite3OsSleep(pVfs, ms<0 ? 0 : 1000*ms)/1000);
  return rc;
}
...
#endif
  sqlite3_mutex_enter(db->mutex);
  Btree *const pBtree = sqlite3DbNameToBtree(db, zDbName);
  if( pBtree ){
    Pager *pPager;
    sqlite3_file *fd;
...
const char *sqlite3_create_filename(
  ...
  pResult = p = static_cast<char *>(sqlite3_malloc64( nByte ));
  if( p==0 ) return 0;
  memset(p, 0, 4);
  p += 4;
...
#endif
  Btree *const pBt = sqlite3DbNameToBtree(db, zDbName);
...
int sqlite3_snapshot_recover(sqlite3 *db, const char *zDb){
  ...
  sqlite3_mutex_enter(db->mutex);
  const int iDb = sqlite3FindDbName(db, zDb);
  if( iDb==0 || iDb>1 ){
    Btree *pBt = db->aDb[iDb].pBt;
...
int sqlite3_compileoption_used(const char *zOptName){
  ...
  int nOpt;
  const char **const azCompileOpt = sqlite3CompileOptions(&nOpt);

  if( sqlite3StrNICmp(zOptName, "SQLITE_", 7)==0 ) zOptName += 7;
  const int n = sqlite3Strlen30(zOptName);
