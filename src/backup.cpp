  ...
  for(iOff=iEnd-(i64)nSrcPgsz; rc==SQLITE_OK && iOff<iEnd; iOff+=nDestPgsz){
    ...
    if( SQLITE_OK==(rc = sqlite3PagerGet(pDestPager, iDest, &pDestPg, 0))
     && SQLITE_OK==(rc = sqlite3PagerWrite(pDestPg))
      ...
      u8 *zDestData = static_cast<u8*>(sqlite3PagerGetData(pDestPg));
      ...
      */
      ...
      (static_cast<u8*>(sqlite3PagerGetExtra(pDestPg)))[0] = 0;
      if( iOff==0 && bUpdate==0 ){
    ...
    */
    ...
    for(ii=0; (nPage<0 || ii<nPage) && p->iNext<=(Pgno)nSrcPage && !rc; ii++){
      ...
      if( iSrcPg!=PENDING_BYTE_PAGE(p->pSrc->pBt) ){
        ...
        if( rc==SQLITE_OK ){
          rc = backupOnePage(p, iSrcPg, static_cast<const u8*>(sqlite3PagerGetData(pSrcPg)), 0);
        ...
        if( pgszSrc<pgszDest ){
          ...
          ** journal file.  */
          ...
          for(
            ...
            if( rc==SQLITE_OK ){
              u8 *zData = static_cast<u8*>(sqlite3PagerGetData(pSrcPg));
...
int sqlite3BtreeCopyFile(Btree *pTo, Btree *pFrom){
  ...
  sqlite3_file *const pFd = sqlite3PagerFile(sqlite3BtreePager(pTo));
  if( pFd->pMethods ){
