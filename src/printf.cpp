    ...
    */
    ...
    switch( xtype ){
      ...
      case etGENERIC: {
        ...
        if( s.isSpecial ){
          if( s.isSpecial==2 ){
            bufpt = const_cast<char*>(flag_zeropad ? "null" : "NaN");
      ...
      case etESCAPE_w: {        /* %w: Escape " characters */
        ...
        if( escarg==0 ){
          escarg = const_cast<char*>((xtype==etESCAPE_Q ? "NULL" : "(NULL)"));
...
int sqlite3StrAccumEnlarge(StrAccum *p, i64 N){
  ...
  if( p->mxAlloc==0 ){
    ...
    if( p->db ){
      ...
      zNew = static_cast<char *>(sqlite3Realloc(zOld, p->nAlloc));
    }
    if( zNew ){
...
sqlite3_str *sqlite3_str_new(sqlite3 *db){
  sqlite3_str *p = static_cast<sqlite3_str *>(sqlite3_malloc64(sizeof(*p)));
  if( p ){
...
char *sqlite3RCStrNew(u64 N){
  RCStr *p = static_cast<RCStr *>(sqlite3_malloc64( N + sizeof(*p) + 1 ));
  if( p==0 ) return 0;
  p->nRCRef = 1;
  return (char*)&p[1];
}
