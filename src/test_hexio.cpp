...
static int SQLITE_TCLAPI hexio_read(
  ...
  zBuf = (unsigned char*)sqlite3_malloc( amt*2+1 );
  if( zBuf==0 ){
    return TCL_ERROR;
  }
  in = fopen(zFile, "rb");
...
static int SQLITE_TCLAPI hexio_write(
  ...
  aOut = (unsigned char*)sqlite3_malloc64( 1 + nIn/2 );
  if( aOut==0 ){
    return TCL_ERROR;
  }
...
static int SQLITE_TCLAPI utf8_to_utf8(
  ...
  z = (unsigned char*)sqlite3_malloc64( n+4 );
  ...
  for(i=0; i<(int)nArg; i++){
    ...
    if( TCL_OK==Tcl_GetWideIntFromObj(0, aArg[i], &iVal) ){
      if( nOut+10>nAlloc ){
        ...
        unsigned char *aNew = (unsigned char*)sqlite3_realloc(aOut, nNew);
        if( aNew==0 ){
          sqlite3_free(aOut);
          return TCL_ERROR;
        }
      ...
      while( (nOut + nVal)>nAlloc ){
        ...
        unsigned char *aNew = (unsigned char*)sqlite3_realloc64(aOut, nNew);
        if( aNew==0 ){
          sqlite3_free(aOut);
          return TCL_ERROR;
        }
