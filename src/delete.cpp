    ...
    */
    ...
    if( eOnePass!=ONEPASS_OFF ){
      ...
      ** delete code.  */
      ...
      aToOpen = static_cast<u8*>(sqlite3DbMallocRawNN(db, nIdx+2));
      if( aToOpen==0 ){
  ...
  ** use for the old.* references in the triggers.  */
  if( sqlite3FkRequired(pParse, pTab, 0, 0) || pTrigger ){
    ...
    ** avoid copying the contents of the rowid register.  */
    ...
    /* Populate the OLD.* pseudo-table array. These values will be
...
**
** Return a number which is the first in a block of
...
** avoid unnecessary loads.  If pPrior is not NULL, then it is
** a pointer to a different index for which an index key has just been
** computed into regPrior.  If the current pIdx index is generating
** its key into the same sequence of registers and if pPrior and pIdx share
** a column in common, then the corresponding to that column already
** holds the correct value and the loading of that is skipped.
...
int sqlite3GenerateIndexKey(
  ...
  int regOut,          /* Put the new key into this if not 0 */
