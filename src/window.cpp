...
**
**   When processing the rewritten SELECT statement, code in select.cpp calls
...
**   by the sub-query a sub-routine (OP_Gosub) coded by select.cpp is invoked.
...
**   select.cpp is the same in either case.
**
...
static void row_numberStepFunc(
  ...
  i64 *p = static_cast<i64*>(sqlite3_aggregate_context(pCtx, sizeof(*p)));
  if( p ) (*p)++;
...
**
** See also the incrAggDepth() routine in resolve.cpp
*/
...
Window *sqlite3WindowAlloc(
  ...
  */
  ...
  pWin = static_cast<Window*>(sqlite3DbMallocZero(pParse->db, sizeof(Window)));
...
/*
** This is called by code in select.cpp before it calls sqlite3WhereBegin()
...
** evaluated and the result left in reg. This function generates VM
...
static void windowCheckValue(Parse *pParse, int reg, int eCond){
  ...
  sqlite3VdbeAppendP4(v, reinterpret_cast<void*>(const_cast<char*>(azErr[eCond])), P4_STATIC);
...
**
** regArg:
**   Each window function requires an accumulator (just as an
...
** the first in an array of registers guaranteed to be large
...
**
** If argument regPartSize is non-zero, then it is a containing the
    ...
    if( pMWin->regStartRowid==0
     ...
     && (pWin->eStart!=TK_UNBOUNDED)
      ...
      if( pFunc->funcFlags & SQLITE_FUNC_NEEDCOLL ){
        ...
        sqlite3VdbeAddOp4(v, OP_CollSeq, 0,0,0, reinterpret_cast<const char*>(pColl), P4_COLLSEQ);
      }
...
static void windowFullScan(WindowCodeArg *p){
  ...
  if( pMWin->eExclude==TK_CURRENT ){
    ...
    if( pKeyInfo ){
      ...
      sqlite3VdbeAppendP4(v, static_cast<void*>(pKeyInfo), P4_KEYINFO);
...
/*
** Invoke the sub-routine at regGosub (generated by code in select.cpp) to
...
/*
** Generate code to set the accumulator for each window function
...
/*
** regOld and regNew are each the first in an array of size
...
static void windowCodeRangeTest(
  ...
  /* Read the peer-value from each cursor into a */
  ...
  ** reg1 or reg2 contain a NULL value.  */
  ...
  sqlite3VdbeAppendP4(v, static_cast<void*>(pColl), P4_COLLSEQ);
...
Window *sqlite3WindowDup(sqlite3 *db, Expr *pOwner, Window *p){
  Window *pNew = 0;
  if( ALWAYS(p) ){
    pNew = static_cast<Window *>(sqlite3DbMallocZero(db, sizeof(Window)));
    if( pNew ){
...
** code to populate the Window.regResult for each window function
