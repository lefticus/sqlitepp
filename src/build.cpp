...
static SQLITE_NOINLINE void lockTable(
  ...
  pToplevel->aTableLock = static_cast<TableLock*>(sqlite3DbReallocOrFree(pToplevel->db, pToplevel->aTableLock, nBytes));
...
Table *sqlite3FindTable(sqlite3 *db, const char *zName, const char *zDatabase){
  ...
  if( zDatabase ){
      ...
      if( i==1 ){
        if( sqlite3StrICmp(zName+7, &PREFERRED_TEMP_SCHEMA_TABLE[7])==0
         ...
         || sqlite3StrICmp(zName+7, &LEGACY_SCHEMA_TABLE[7])==0
          ...
          p = static_cast<Table*>(sqlite3HashFind(&db->aDb[1].pSchema->tblHash,
                              LEGACY_TEMP_SCHEMA_TABLE));
        }
      }else{
        if( sqlite3StrICmp(zName+7, &PREFERRED_SCHEMA_TABLE[7])==0 ){
          p = static_cast<Table*>(sqlite3HashFind(&db->aDb[i].pSchema->tblHash,
                              LEGACY_SCHEMA_TABLE));
        }
      }
    }
    ...
    p = static_cast<Table*>(sqlite3HashFind(&db->aDb[1].pSchema->tblHash, zName));
    if( p ) return p;
    ...
    if( p==0 && sqlite3StrNICmp(zName, "sqlite_", 7)==0 ){
      if( sqlite3StrICmp(zName+7, &PREFERRED_SCHEMA_TABLE[7])==0 ){
        p = static_cast<Table*>(sqlite3HashFind(&db->aDb[0].pSchema->tblHash, LEGACY_SCHEMA_TABLE));
  ...
  if( p==0 ){
    ...
    ** can be an eponymous virtual table. */
    if( (pParse->prepFlags & SQLITE_PREPARE_NO_VTAB)==0 && db->init.busy==0 ){
      Module *pMod = static_cast<Module*>(sqlite3HashFind(&db->aModule, zName));
...
Index *sqlite3FindIndex(sqlite3 *db, const char *zName, const char *zDb){
  ...
  for(int i=OMIT_TEMPDB; i<db->nDb; i++){
    ...
    p = static_cast<Index*>(sqlite3HashFind(&pSchema->idxHash, zName));
    if( p ) break;
  }
  return p;
}
...
void sqlite3FreeIndex(sqlite3 *db, Index *p){
  ...
  if( p->isResized ) sqlite3DbFree(db, reinterpret_cast<void*>(const_cast<char**>(p->azColl)));
...
void sqlite3ColumnSetColl(
  ...
  zNew = static_cast<char*>(sqlite3DbRealloc(db, pCol->zCnName, nColl+n));
  if( zNew ){
    pCol->zCnName = zNew;
  ...
  ** that no lookaside memory is used in this case either. */
  ...
  for(Index *pIndex = pTable->pIndex, *pNext; pIndex; pIndex=pNext){
    ...
    if( db->pnBytesFreed==0 && !IsVirtual(pTable) ){
      ...
      TESTONLY ( Index *pOld = ) static_cast<Index*>(sqlite3HashInsert(
...
void sqlite3StartTable(
  ...
  if( db->init.busy && db->init.newTnum==1 ){
    ...
    if( IN_RENAME_OBJECT ){
      sqlite3RenameTokenMap(pParse, static_cast<const void*>(zName), pName);
    }
  }
  ...
  */
  ...
  pTable = static_cast<Table*>(sqlite3DbMallocZero(db, sizeof(Table)));
  if( pTable==0 ){
  ...
  */
  if( !db->init.busy && (v = sqlite3GetVdbe(pParse))!=0 ){
    ...
    **
    ** The rowid for the new entry is left in pParse->u1.cr.regRowid.
...
void sqlite3EndTable(
  ...
  */
  if( !db->init.busy ){
    ...
    ** new table is in pParse->u1.cr.regRoot.
    **
  ...
  */
  if( db->init.busy ){
    ...
    assert( sqlite3SchemaMutexHeld(db, iDb, 0) );
    assert( HasRowid(p) || p->iPKey<0 );
    pOld = static_cast<Table*>(sqlite3HashInsert(&pSchema->tblHash, p->zName, p));
    if( pOld ){
...
void sqlite3RootPageMoved(sqlite3 *db, int iDb, Pgno iFrom, Pgno iTo){
  ...
  for(pElem=sqliteHashFirst(pHash); pElem; pElem=sqliteHashNext(pElem)){
    Index *pIdx = static_cast<Index*>(sqliteHashData(pElem));
    if( pIdx->tnum==iFrom ){
      pIdx->tnum = iTo;
    }
  }
...
static void destroyRootPage(Parse *pParse, int iTable, int iDb){
  ...
  ** is in NNN.  See grammar rules associated with the TK_REGISTER
...
void sqlite3CreateForeignKey(
  ...
  pFKey = static_cast<FKey*>(sqlite3DbMallocZero(db, nByte ));
  if( pFKey==0 ){
    goto fk_end;
  }
  pFKey->pFrom = p;
  ...
  pFKey->zTo = z;
  if( IN_RENAME_OBJECT ){
    sqlite3RenameTokenMap(pParse, static_cast<const void*>(z), pTo);
  }
  ...
  pNextTo = static_cast<FKey*>(sqlite3HashInsert(&p->pSchema->fkeyHash,
      pFKey->zTo, static_cast<void*>(pFKey)
  ));
  if( pNextTo==pFKey ){
    sqlite3OomFault(db);
    goto fk_end;
...
** created.  The specified by memRootPage contains the
...
Index *sqlite3AllocateIndexObject(
  ...
  p = static_cast<Index*>(sqlite3DbMallocZero(db, nByte + nExtra));
  if( p ){
  ...
  if( !IN_RENAME_OBJECT ){
    ...
    */
    ...
    if( db->init.busy ){
      ...
      p = static_cast<Index*>(sqlite3HashInsert(&pIndex->pSchema->idxHash,
          pIndex->zName, pIndex));
      if( p ){
...
IdList *sqlite3IdListAppend(Parse *pParse, IdList *pList, Token *pToken){
  ...
  if( pList==0 ){
    ...
    IdList *pNew;
    pNew = static_cast<IdList*>(sqlite3DbRealloc(db, pList, SZ_IDLIST(pList->nId+1)));
    if( pNew==0 ){
...
SrcList *sqlite3SrcListEnlarge(
  ...
  if( (u32)pSrc->nSrc+nExtra>pSrc->nAlloc ){
    ...
    pNew = static_cast<SrcList*>(sqlite3DbRealloc(db, pSrc, SZ_SRCLIST(nAlloc)));
    if( pNew==0 ){
      assert( db->mallocFailed );
      return 0;
    }
...
SrcList *sqlite3SrcListAppend(
  ...
  db = pParse->db;
  if( pList==0 ){
    pList = static_cast<SrcList*>(sqlite3DbMallocRawNN(pParse->db, SZ_SRCLIST(1)));
    if( pList==0 ) return 0;
    pList->nAlloc = 1;
    pList->nSrc = 1;
...
int sqlite3SrcItemAttachSubquery(
  ...
  p = pItem->u4.pSubq = static_cast<Subquery*>(sqlite3DbMallocRawNN(pParse->db, sizeof(Subquery)));
  if( p==0 ){
...
Cte *sqlite3CteNew(
  ...
  pNew = static_cast<Cte*>(sqlite3DbMallocZero(db, sizeof(*pNew)));
...
With *sqlite3WithAdd(
  ...
  ** not, store an error in the Parse structure. */
  ...
  if( pWith ){
    pNew = static_cast<With*>(sqlite3DbRealloc(db, pWith, SZ_WITH(pWith->nCte+1)));
  }else{
    pNew = static_cast<With*>(sqlite3DbMallocZero(db, SZ_WITH(1)));
  }
