...
static void pager_unlock(Pager *pPager){
  ...
  if( pagerUseWal(pPager) ){
    ...
    if( pPager->eState==PAGER_ERROR ){
      /* If an IO error occurs in wal.cpp while attempting to wrap the wal file,
...
static int pager_delsuper(Pager *pPager, const char *zSuper){
  ...
  */
  ...
  zFree = static_cast<char*>(sqlite3Malloc(4 + nSuperJournal + nSuperPtr + 2));
  if( !zFree ){
    rc = SQLITE_NOMEM_BKPT;
    goto delsuper_out;
  }else{
...
static int pager_playback(Pager *pPager, int isHot){
  ...
  ** (pPager->pageSize >= pPager->pVfs->mxPathname+1). Using os_unix.cpp,
...
static int readDbPage(PgHdr *pPg){
  ...
  if( iFrame ){
    rc = sqlite3WalReadFrame(pPager->pWal, iFrame,pPager->pageSize,static_cast<u8*>(pPg->pData));
  }else
#endif
  {
   ...
   && pageSize && pageSize!=(u32)pPager->pageSize
    ...
    if( rc==SQLITE_OK ){
      ...
      * cell header parser will never run off the end of the allocation */
      pNew = static_cast<char*>(sqlite3PageMalloc(pageSize+8));
      if( !pNew ){
        rc = SQLITE_NOMEM_BKPT;
      }else{
...
static int pagerAcquireMapPage(
  ...
  if( pPager->pMmapFreelist ){
    ...
    *ppPage = p = static_cast<PgHdr*>(sqlite3MallocZero(sizeof(PgHdr) + pPager->nExtra));
    if( p==0 ){
  ...
  while( rc==SQLITE_OK && pList ){
    ...
    */
    if( pgno<=pPager->dbSize && 0==(pList->flags&PGHDR_DONT_WRITE) ){
      ...
      pData = static_cast<char*>(pList->pData);
...
int sqlite3PagerOpen(
  ...
  */
  if( zFilename && zFilename[0] ){
    ...
    zPathname = static_cast<char*>(sqlite3DbMallocRaw(0, 2*(i64)nPathname));
    if( zPathname==0 ){
      return SQLITE_NOMEM_BKPT;
    }
...
act_like_temp_file:
  ...
  */
  ...
  /* pPager->szMmap = SQLITE_DEFAULT_MMAP_SIZE // will be set by btree.cpp */
...
static SQLITE_NOINLINE int pagerAddPageToRollbackJournal(PgHdr *pPg){
  ...
  ** that we do not. */
  ...
  assert( pPager->journalHdr<=pPager->journalOff );
  pData2 = static_cast<char*>(pPg->pData);
...
static SQLITE_NOINLINE int pagerOpenSavepoint(Pager *pPager, int nSavepoint){
  ...
  */
  aNew = static_cast<PagerSavepoint*>(sqlite3Realloc(
...
** in backup.cpp maintains the content of this variable. This module
