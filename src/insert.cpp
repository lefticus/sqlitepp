...
** affinities for iReg and following.  Or if iReg==0,
...
** set as the OP_MakeRecord.  If iReg>0 then iReg is
...
** which is in database iDb.  Return the number for the register
...
**
** The 2nd is the one that is returned.  That is all the
...
static int autoIncBegin(
  ...
  if( (pTab->tabFlags & TF_Autoincrement)!=0
    ...
    ** Ticket d8dc2b3a58cd5dc2918a1d4acb 2018-05-23 */
    ...
    if( pInfo==0 ){
      pInfo = static_cast<AutoincInfo*>(sqlite3DbMallocRawNN(pParse->db, sizeof(*pInfo)));
      ...
      pInfo->regCtr = ++pToplevel->nMem;  /* Max rowid */
...
**
** This routine should be called when the regRowid holds a
...
**       sqlite3SubqueryColumnTypes() in select.cpp) to determine the effective
...
void sqlite3Insert(
  ...
  int regFromSelect = 0;/* Base for data coming from SELECT */
  int regAutoinc = 0;   /* Register holding the AUTOINCREMENT counter */
  int regRowCount = 0;  /* Memory cell used for the row counter */
  int regIns;           /* Block of regs holding rowid+data being inserted */
  int regRowid;         /* registers holding insert rowid */
  int regData;          /* holding first column to insert */
  int *aRegIdx = 0;     /* One allocated to each index */
...
# define isView 0
  ...
  */
  ...
  if( pColumn ){
    aTabColMap = static_cast<int*>(sqlite3DbMallocZero(db, pTab->nCol*sizeof(int)));
  ...
  */
  ...
  if( !isView ){
    ...
    nIdx = sqlite3OpenTableAndIndices(pParse, pTab, OP_OpenWrite, 0, -1, 0,
                                      &iDataCur, &iIdxCur);
    aRegIdx = static_cast<int*>(sqlite3DbMallocRawNN(db, sizeof(int)*(nIdx+2)));
    if( aRegIdx==0 ){
      goto insert_cleanup;
    }
  ...
  for(i=0; i<pTab->nCol; i++, iRegStore++){
    ...
    if( ((colFlags = pTab->aCol[i].colFlags) & COLFLAG_NOINSERT)!=0 ){
      ...
      if( (colFlags & COLFLAG_VIRTUAL)!=0 ){
        ...
        ** to a second block of registers, so the needs to be
        ** initialized to NULL to avoid an uninitialized read */
  ...
  */
  ...
  if( !isView ){
    ...
    */
    ...
    /* If there is a RETURNING clause, populate the rowid with
...
** on table pTab.
**
** The regNewData parameter is the first in a range that contains
** the data to be inserted or the data after the update.  There will be
** pTab->nCol+1 registers in this range.  The first (the one
** that regNewData points to) will contain the new rowid, or NULL in the
** case of a WITHOUT ROWID table.  The second in the range will
** contain the content of the first table column.  The third will
...
** identified by aRegIdx[nIdx] - in other words in the first
** entry of aRegIdx[] past the last index.  It is important that the
** record be generated during constraint checks to avoid affinity changes
** to the content that occur after constraint checks but before
...
void sqlite3GenerateConstraintChecks(
  ...
  int *aRegIdx,        /* Use aRegIdx[i] for index i.  0 for unused */
  int iDataCur,        /* Canonical data cursor (main table or PK index) */
  int iIdxCur,         /* First index cursor */
  int regNewData,      /* First in a range holding values to insert */
  ...
  if( pUpsert ){
    if( pUpsert->pUpsertTarget==0 ){
      ...
      ** checked first and in order. */
      ...
      sIdxIter.u.ax.aIdx = static_cast<IndexListTerm*>(sqlite3DbMallocZero(db, nByte));
  ...
  **
  **   (1) Allocate regTrigCnt and initialize it to zero.
  **       That will count the number of replace triggers that
          ...
          while( nConflictCk>0 ){
            ...
            ** a pointer to the opcode. */
            ...
            if( x.opcode!=OP_IdxRowid ){
              ...
              zP4 = x.p4type==P4_INT32 ? reinterpret_cast<const char*>(SQLITE_INT_TO_PTR(x.p4.i)) : x.p4.z;
...
** number is iCur, and regData contains the new record for the
...
static int xferOptimization(
  ...
  int regAutoinc;                  /* Memory used by AUTOINC */
