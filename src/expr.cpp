...
int sqlite3ExprCollSeqMatch(Parse *pParse, const Expr *pE1, const Expr *pE2){
  const CollSeq *pColl1 = sqlite3ExprNNCollSeq(pParse, pE1);
  const CollSeq *pColl2 = sqlite3ExprNNCollSeq(pParse, pE2);
...
static char comparisonAffinity(const Expr *pExpr){
  ...
  assert( pExpr->pLeft );
  char aff = sqlite3ExprAffinity(pExpr->pLeft);
  if( pExpr->pRight ){
...
int sqlite3IndexAffinityOk(const Expr *pExpr, char idx_affinity){
  const char aff = comparisonAffinity(pExpr);
...
static int codeCompare(
  ...
  const int p5 = binaryCompareP5(pLeft, pRight, jumpIfNull);
  const int addr = sqlite3VdbeAddOp4(pParse->pVdbe, opcode, in2, dest, in1,
                           reinterpret_cast<const char*>(p4), P4_COLLSEQ);
...
Expr *sqlite3ExprForVectorField(
  ...
  if( pVector->op==TK_SELECT ){
    ...
    ** pLeft->iTable:   First in an array of holding result, or 0
...
** it. Return the in which the result is stored (or, if the
...
** the number of a that contains the value of
...
** a temporary to be freed by the caller before returning.
**
...
static void codeVectorCompare(
  ...
  int dest,             /* Write results into this */
  u8 op,                /* Comparison operator */
  u8 p5                 /* SQLITE_NULLEQ or zero */
){
  Vdbe *const v = pParse->pVdbe;
  Expr *const pLeft = pExpr->pLeft;
  Expr *const pRight = pExpr->pRight;
  const int nLeft = sqlite3ExprVectorSize(pLeft);
  int regLeft = 0;
  int regRight = 0;
  u8 opx = op;
  int addrCmp = 0;
  const int addrDone = sqlite3VdbeMakeLabel(pParse);
  const int isCommuted = ExprHasProperty(pExpr,EP_Commuted);
  ...
  for(int i=0; 1 /*Loop exits by "break"*/; i++){
...
int sqlite3ExprCheckHeight(Parse *pParse, int nHeight){
  int rc = SQLITE_OK;
  const int mxHeight = pParse->db->aLimit[SQLITE_LIMIT_EXPR_DEPTH];
  if( nHeight>mxHeight ){
...
Expr *sqlite3ExprAlloc(
  ...
  Expr *pNew;
  const int nExtra = pToken ? pToken->n+1 : 0;

  assert( db!=0 );
  pNew = static_cast<Expr*>(sqlite3DbMallocRawNN(db, sizeof(Expr)+nExtra));
  if( pNew ){
...
Expr *sqlite3ExprInt32(sqlite3 *db, int iVal){
  Expr *pNew = static_cast<Expr*>(sqlite3DbMallocRawNN(db, sizeof(Expr)));
  if( pNew ){
    memset(pNew, 0, sizeof(Expr));
    pNew->op = TK_INTEGER;
    pNew->iAgg = -1;
...
Expr *sqlite3PExpr(
  ...
  Expr *p;
  p = static_cast<Expr*>(sqlite3DbMallocRawNN(pParse->db, sizeof(Expr)));
  if( p ){
...
Expr *sqlite3ExprAnd(Parse *pParse, Expr *pLeft, Expr *pRight){
  ...
  if( pLeft==0  ){
    ...
    const u32 f = pLeft->flags | pRight->flags;
...
static Expr *exprDup(
  ...
  if( pEdupBuf ){
    ...
    sEdupBuf.zAlloc = static_cast<u8*>(sqlite3DbMallocRawNN(db, nAlloc));
#ifdef SQLITE_DEBUG
...
With *sqlite3WithDup(sqlite3 *db, With *p){
  With *pRet = 0;
  if( p ){
    const sqlite3_int64 nByte = SZ_WITH(p->nCte);
    pRet = static_cast<With*>(sqlite3DbMallocZero(db, nByte));
    if( pRet ){
      int i;
      pRet->nCte = p->nCte;
      for(i=0; i<p->nCte; i++){
...
ExprList *sqlite3ExprListDup(sqlite3 *db, const ExprList *p, int flags){
  ...
  assert( db!=0 );
  if( p==0 ) return 0;
  pNew = static_cast<ExprList*>(sqlite3DbMallocRawNN(db, sqlite3DbMallocSize(db, p)));
  if( pNew==0 ) return 0;
  pNew->nExpr = p->nExpr;
...
SrcList *sqlite3SrcListDup(sqlite3 *db, const SrcList *p, int flags){
  ...
  assert( db!=0 );
  if( p==0 ) return 0;
  pNew = static_cast<SrcList*>(sqlite3DbMallocRawNN(db, SZ_SRCLIST(p->nSrc) ));
  if( pNew==0 ) return 0;
  ...
  for(i=0; i<p->nSrc; i++){
    ...
    if( pOldItem->fg.isSubquery ){
      Subquery *pNewSubq = static_cast<Subquery*>(sqlite3DbMallocRaw(db, sizeof(Subquery)));
      if( pNewSubq==0 ){
        assert( db->mallocFailed );
...
IdList *sqlite3IdListDup(sqlite3 *db, const IdList *p){
  ...
  assert( db!=0 );
  if( p==0 ) return 0;
  pNew = static_cast<IdList*>(sqlite3DbMallocRawNN(db, SZ_IDLIST(p->nId)));
  if( pNew==0 ) return 0;
  pNew->nId = p->nId;
  for(i=0; i<p->nId; i++){
...
Select *sqlite3SelectDup(sqlite3 *db, const Select *pDup, int flags){
  ...
  for(p=pDup; p; p=p->pPrior){
    Select *pNew = static_cast<Select*>(sqlite3DbMallocRawNN(db, sizeof(*p) ));
    if( pNew==0 ) break;
...
SQLITE_NOINLINE ExprList *sqlite3ExprListAppendNew(
  ...
  pList = static_cast<ExprList*>(sqlite3DbMallocRawNN(db, SZ_EXPRLIST(4)));
  if( pList==0 ){
...
SQLITE_NOINLINE ExprList *sqlite3ExprListAppendGrow(
  ...
  ExprList *pNew;
  pList->nAlloc *= 2;
  pNew = static_cast<ExprList*>(sqlite3DbRealloc(db, pList, SZ_EXPRLIST(pList->nAlloc)));
  if( pNew==0 ){
...
void sqlite3ExprListSetSortOrder(ExprList *p, int iSortOrder, int eNulls){
  ...
  struct ExprList_item *const pItem = &p->a[p->nExpr-1];
...
void sqlite3ExprListCheckLength(
  ...
  const int mx = pParse->db->aLimit[SQLITE_LIMIT_COLUMN];
...
static int exprComputeOperands(
  ...
  int *pFree1,       /* OUT: Temp to free if not zero */
  int *pFree2        /* OUT: Another temp to free if not zero */
){
  int addrIsNull;
  int r1, r2;
  Vdbe *v = pParse->pVdbe;
...
static SQLITE_NOINLINE int exprNodeIsConstantFunction(
  ...
  sqlite3 *const db = pWalker->pParse->db;
  FuncDef *const pDef = sqlite3FindFunction(db, pExpr->u.zToken, n, ENC(db), 0);
  if( pDef==0
   || pDef->xFinalize!=0
...
static Select *isCandidateForInOpt(const Expr *pX){
  ...
  Table *const pTab = pSrc->a[0].pSTab;
  assert( pTab!=0 );
  assert( !IsView(pTab)  );              /* FROM clause is not a view */
  if( IsVirtual(pTab) ) return 0;        /* FROM clause not a virtual table */
  ExprList *const pEList = p->pEList;
  assert( pEList!=0 );
...
** it contains any NULL entries.  Cause the at regHasNull to be set
** to a non-NULL value if iCur contains no NULLs.  Cause regHasNull
** to be set to NULL if iCur contains one or more NULL values.
*/
static void sqlite3SetHasNullFlag(Vdbe *v, int iCur, int regHasNull){
  sqlite3VdbeAddOp2(v, OP_Integer, 0, regHasNull);
  const int addr1 = sqlite3VdbeAddOp1(v, OP_Rewind, iCur); VdbeCoverage(v);
...
** runtime, then a is allocated and the number written
** to *prRhsHasNull. If there is no chance that the (...) contains a
** NULL value, then *prRhsHasNull is left unchanged.
**
** If a is allocated and its location stored in *prRhsHasNull, then
** the value in that will be NULL if the b-tree contains one or more
...
static char *exprINAffinity(Parse *pParse, const Expr *pExpr){
  Expr *const pLeft = pExpr->pLeft;
  const int nVal = sqlite3ExprVectorSize(pLeft);
  Select *const pSelect = ExprUseXSelect(pExpr) ? pExpr->x.pSelect : 0;
  char *zRet;

  assert( pExpr->op==TK_IN );
  zRet = static_cast<char*>(sqlite3DbMallocRaw(pParse->db, 1+(i64)nVal));
  if( zRet ){
    int i;
    for(i=0; i<nVal; i++){
...
static int findCompatibleInRhsSubrtn(
  ...
  VdbeOp *const pEnd = sqlite3VdbeGetLastOp(v);
  for(; pOp<pEnd; pOp++){
    if( pOp->p4type!=P4_SUBRTNSIG ) continue;
    assert( pOp->opcode==OP_BeginSubrtn );
    SubrtnSig *const pSig = pOp->p4.pSubrtnSig;
    assert( pSig!=0 );
...
void sqlite3CodeRhsOfIN(
  ...
  if( !ExprHasProperty(pExpr, EP_VarSelect) && pParse->iSelfTab==0 ){
    ...
    */
    ...
    if( ExprUseXSelect(pExpr) && (pExpr->x.pSelect->selFlags & SF_All)==0 ){
      pSig = static_cast<SubrtnSig*>(sqlite3DbMallocRawNN(pParse->db, sizeof(pSig[0])));
      if( pSig ){
  ...
  */
  ...
  if( pKeyInfo ){
    sqlite3VdbeChangeP4(v, addr, reinterpret_cast<const char*>(pKeyInfo), P4_KEYINFO);
  }
  if( addrOnce ){
...
**
** Return the that holds the result.  For a multi-column SELECT,
** the result is stored in a contiguous array of registers and the
** return value is the of the left-most result column.
...
int sqlite3ExprCheckIN(Parse *pParse, Expr *pIn){
  const int nVector = sqlite3ExprVectorSize(pIn->pLeft);
...
static void sqlite3ExprCodeIN(
  ...
  aiMap = static_cast<int*>(sqlite3DbMallocZero(pParse->db, nVector*sizeof(int)));
    ...
    for(ii=0; ii<pList->nExpr; ii++){
      ...
      if( ii<pList->nExpr-1 || destIfNull!=destIfFalse ){
        ...
        sqlite3VdbeAddOp4(v, op, rLhs, labelOk, r2,
                          reinterpret_cast<const char*>(pColl), P4_COLLSEQ);
...
** value described by z[0..n-1] into iMem.
**
...
/* Generate code that will load into regOut a value that is
...
void sqlite3ExprCodeLoadIndexColumn(
  ...
  int regOut      /* Store the index column value in this */
){
...
void sqlite3ExprCodeGetColumnOfTable(
  ...
  int regOut      /* Extract the value into this */
){
  Column *pCol;
  assert( v!=0 );
  assert( pTab!=0 );
...
** table pTab and store the column value in iReg.
**
...
** iReg.  The caller must ensure that iReg already contains
...
** the first used to store the result.
**
** If the returned result is a temporary scalar, then also write
** that number into *piFreeable.  If the returned result register
** is not a temporary or if the expression is a vector set *piFreeable
** to 0.
*/
static int exprCodeVector(Parse *pParse, Expr *p, int *piFreeable){
  int iResult;
  const int nResult = sqlite3ExprVectorSize(p);
  if( nResult==1 ){
...
static int exprCodeInlineFunction(
  ...
  int target            /* Store function result in this */
){
  Vdbe *const v = pParse->pVdbe;
  assert( v!=0 );
  assert( pFarg!=0 );
  const int nFarg = pFarg->nExpr;
...
static int exprNodeCanReturnSubtype(Walker *pWalker, Expr *pExpr){
  ...
  assert( ExprUseXList(pExpr) );
  sqlite3 *const db = pWalker->pParse->db;
  const int n = ALWAYS(pExpr->x.pList) ? pExpr->x.pList->nExpr : 0;
  FuncDef *const pDef = sqlite3FindFunction(db, pExpr->u.zToken, n, ENC(db), 0);
...
** return the into which the value has been read.  If pExpr is
...
** put the constant value in a (ideally, but not necessarily, 
** iTarget) and returns the number.
**
...
static SQLITE_NOINLINE int exprCodeTargetAndOr(
  ...
  int *pTmpReg       /* Write a temporary here */
){
...
** expression.  Attempt to store the results in "target".
** Return the where results are stored.
**
** With this routine, there is no guarantee that results will
** be stored in target.  The result might be stored in some other
** if it is convenient to do so.  The calling function
...
expr_code_doover:
  ...
  switch( op ){
    ...
    case TK_BLOB: {
      ...
      assert( z[n]=='\'' );
      zBlob = static_cast<char*>(sqlite3HexToBlob(sqlite3VdbeDb(v), z, n));
    ...
    */
    case TK_IF_NULL_ROW: {
      ...
      /* The OP_IfNullRow opcode above can overwrite the result with
      ** NULL.  So we have to ensure that the result is not a value
    ...
    */
    case TK_CASE: {
      ...
      if( (pX = pExpr->pLeft)!=0 ){
        ...
        ** So make sure that the regFree1 is not reused for other
...
**
** If regDest>0 then the result is always stored in that and the
** result is not reusable.  If regDest<0 then this routine is free to
** store the value wherever it wants.  The where the expression
...
int sqlite3ExprCodeRunJustOnce(
  ...
  int regDest       /* Store the value in this */
){
  ExprList *p;
...
** into a register.  Return the number where the results
** are stored.
**
** If the is a temporary that can be deallocated,
** then write its number into *pReg.  If the result is not
** a temporary, then set *pReg to zero.
**
** If pExpr is a constant, then this routine might generate this
** code to fill the in the initialization section of the
...
** results in target.  The results are guaranteed to appear
** in target.
*/
void sqlite3ExprCode(Parse *pParse, Expr *pExpr, int target){
  ...
  const int inReg = sqlite3ExprCodeTarget(pParse, pExpr, target);
  if( inReg!=target ){
    u8 op;
...
** Generate code that will evaluate expression pExpr and store the
** results in target.  The results are guaranteed to appear
** in target.  If the expression is constant, then this routine
...
int sqlite3ExprCodeExprList(
  ...
  struct ExprList_item *pItem;
  int i, j, n;
  const u8 copyOp = (flags & SQLITE_ECEL_DUP) ? OP_Copy : OP_SCopy;
  Vdbe *const v = pParse->pVdbe;
  assert( pList!=0 );
  assert( target>0 );
...
** operation.  Special comments in vdbe.cpp and the mkopcodeh.awk script in
  ...
  switch( pExpr->op ){
    ...
    ** is the column of a virtual table */
    ...
    case TK_GE: {
      ...
      /* The y.pTab=0 assignment in wherecode.cpp always happens after the
...
static int selectRefEnter(Walker *pWalker, Select *pSelect){
  ...
  piNew = static_cast<int*>(sqlite3DbRealloc(p->db, p->aiExclude, p->nExclude*sizeof(int)));
  if( piNew==0 ){
    p->nExclude = 0;
    return WRC_Abort;
  }else{
...
static int addAggInfoColumn(sqlite3 *db, AggInfo *pInfo){
  int i;
  pInfo->aCol = static_cast<struct AggInfo_col*>(sqlite3ArrayAllocate(
       db,
       pInfo->aCol,
       sizeof(pInfo->aCol[0]),
       &pInfo->nColumn,
...
static int addAggInfoFunc(sqlite3 *db, AggInfo *pInfo){
  int i;
  pInfo->aFunc = static_cast<struct AggInfo_func*>(sqlite3ArrayAllocate(
       db,
       pInfo->aFunc,
...
static void findOrCreateAggInfoColumn(
  ...
  struct AggInfo_col *pCol;
  int k;
  const int mxTerm = pParse->db->aLimit[SQLITE_LIMIT_COLUMN];
...
static int analyzeAggregate(Walker *pWalker, Expr *pExpr){
  ...
  SrcList *const pSrcList = pNC->pSrcList;
  AggInfo *const pAggInfo = pNC->uNC.pAggInfo;
...
/*
** Allocate a single new for use to hold some intermediate result.
*/
...
/*
** Return the latest reusable in the set of all registers.
** The value returned is no less than iMin.  If any iMin or
...
/*
** Validate that no temporary falls within the range of
