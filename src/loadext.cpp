...
static int sqlite3LoadExtension(
  ...
  */
  if( xInit==0 && zProc==0 ){
    ...
    int cnt = 0;
    zAltEntry = static_cast<char *>(sqlite3_malloc64(ncFile+30));
    if( zAltEntry==0 ){
  ...
  if( xInit==0 ){
    if( pzErrMsg ){
      ...
      *pzErrMsg = zErrmsg = static_cast<char *>(sqlite3_malloc64(nMsg));
      if( zErrmsg ){
  ...
  aHandle = static_cast<void **>(sqlite3DbMallocZero(db, sizeof(handle)*(db->nExtension+1)));
  if( aHandle==0 ){
    return SQLITE_NOMEM_BKPT;
  }
  if( db->nExtension>0 ){
...
int sqlite3_auto_extension(
...
#endif
  {
    ...
    if( i==wsdAutoext.nExt ){
      ...
      void (**aNew)(void);
      aNew = static_cast<void (**)(void)>(sqlite3_realloc64(wsdAutoext.aExt, nByte));
      if( aNew==0 ){
